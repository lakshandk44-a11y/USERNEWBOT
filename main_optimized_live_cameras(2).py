...
for _i in range(1, 7):
    LIVE_CAMERAS.append({
        ...
        "title": os.getenv(f"LIVE_CAM_{_i}_TITLE", ""),
        "description": os.getenv(f"LIVE_CAM_{_i}_DESCRIPTION", ""),
        "hashtags": os.getenv(f"LIVE_CAM_{_i}_HASHTAGS", ""),
    })
...
def _start_live_camera(index, automatic=False):
    ...
    title = cam.get("title") or f"{LIVE_CAM_TITLE_PREFIX} — {cam['name']}"
    description = cam.get("description") or f"Live view from {cam['location']}. Officially authorized source."
    hashtags = _clean_hashtags(cam.get("hashtags", ""), 5)
    if hashtags:
