...
def build_video_reel(slides, out_path):
    ...
    try:
        ...
        for i, (img_path, slide, duration) in enumerate(zip(img_paths, slides, durations)):
            ...
            # Caption is intentionally kept away from the bottom UI area.
            if _VIDEO_FONT:
        ...
        if music_enabled:
            cmd += ["-stream_loop", "-1", "-i", VIDEO_MUSIC_FILE]
        ...
        if has_audio and music_enabled:
        ...
        # Avoid an accidental duplicate -filter_complex from the initial setup.
        # Rebuild the command cleanly before execution.
        prefix = ["ffmpeg", "-y"]
        for img_path in img_paths:
        ...
        if has_audio:
            final_cmd += ["-map", "[aout]", "-c:a", "aac", "-b:a", "160k"]
        else:
            final_cmd += ["-an"]
        final_cmd += [
            "-t", f"{elapsed:.3f}",
        ...
        r = subprocess.run(final_cmd, capture_output=True, timeout=240)
...
def build_reel_cover(slides, cover_path):
    ...
    try:
        ...
        vf = [
            ...
            "vignette=PI/5",
            "drawbox=x=45:y=610:w=990:h=730:color=black@0.42:t=fill",
        ]
        if _VIDEO_FONT:
            vf.extend([
