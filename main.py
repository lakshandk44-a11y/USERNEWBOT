...
category_fail_streak = {"news": 0, "scenic": 0, "cartoon": 0, "quote": 0, "fact": 0, "poll": 0}
...
def build_settings_keyboard():
    ...
    # Group sharing section (only shown if groups are configured)
    if FB_GROUP_IDS:
        buttons.append([{"text": "─── 👥 GROUP SHARING ───", "callback_data": "noop"}])
        grp_row = []
        ...
        for key, label in grp_labels.items():
            ...
            grp_row.append({"text": f"{icon} {label}→Grp", "callback_data": f"grp_{key}"})
            if len(grp_row) == 2:
...
def send_settings_message():
    if not (TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID):
        return
    grp_info = f"\n👥 Sharing to {len(FB_GROUP_IDS)} group(s)" if FB_GROUP_IDS else "\n👥 No groups configured (set FB_GROUP_IDS)"
    try:
        requests.post(
            f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage",
            json={
                "chat_id": TELEGRAM_CHAT_ID,
                "text": f"⚙️ *Bot Settings*{grp_info}\n\nTap to toggle ON🟢 / OFF🔴:",
                "parse_mode": "Markdown",
...
def call_gemini(prompt, retries=3, label="gemini"):
    """Calls Gemini with automatic RPM rate limiting and Pollinations fallback."""
    ...
    log(f"⚠️ [{label}] Gemini fully failed — trying Pollinations.ai text fallback...")
    fallback_text = _call_pollinations_text(prompt, label=label)
    if fallback_text:
        log(f"✅ [{label}] Pollinations.ai text fallback succeeded.")
        return fallback_text

    log(f"❌ [{label}] All AI sources failed (Gemini + Pollinations). Returning None.")
    return None
...
# ================= GROUP AUTO-SHARE (NEW) =================
# Posts the FULL content (caption + image) directly into each group,
# exactly as it appears on the Page — not a link share.
# Per-category control via GROUP_SHARE_ENABLED (toggled via /settings).
# Handles approval-required groups gracefully (they go to pending queue).
def share_to_groups(post_id, caption, image, post_type):
    if not FB_GROUP_IDS:
        return
    if not GROUP_SHARE_ENABLED.get(post_type, False):
        return   # this category is turned off for groups

    img_bytes = image.get("bytes") if image else None
    shared = []
    pending = []
    failed = []
    ...
    for group_id in FB_GROUP_IDS:
        try:
            if img_bytes:
                # Full image + caption post — identical visual to the Page post
                resp = requests.post(
                    f"https://graph.facebook.com/v20.0/{group_id}/photos",
                    data={"caption": caption, "access_token": FB_USER_TOKEN},
                ...
                # Text-only fallback
                resp = requests.post(
                    f"https://graph.facebook.com/v20.0/{group_id}/feed",
                    data={"message": caption, "access_token": FB_USER_TOKEN},
                    timeout=REQUEST_TIMEOUT,
                ).json()

            if "error" in resp:
                err_msg = resp["error"].get("message", "")
                # Some groups require admin approval — not a real error,
                # post goes to pending queue and admins approve it.
                if any(w in err_msg.lower() for w in ["pending", "approv", "review", "not allowed"]):
                    pending.append(group_id)
                else:
                    failed.append(f"{group_id}: {err_msg[:60]}")
            else:
                shared.append(group_id)

        except Exception as e:
            failed.append(f"{group_id}: {str(e)[:60]}")

        # 2-3 min random delay between groups — prevents Facebook duplicate
        # detector from flagging identical content hitting multiple groups
        # at the same timestamp. Skip delay after the last group.
        if group_id != FB_GROUP_IDS[-1]:
            delay = random.randint(120, 180)
            log(f"⏳ Next group in {delay}s...")
            time.sleep(delay)
    if shared:  parts.append(f"✅ Posted to {len(shared)} group(s)")
    if pending: parts.append(f"⏳ Pending approval in {len(pending)} group(s)")
    if failed:  parts.append(f"❌ Failed: {'; '.join(failed)}")
    if parts:
        log(f"👥 Groups [{post_type}]: {' | '.join(parts)}")
        ...
        try:
            ...
            if "error" not in result:
                ...
                if "id" in result:
                    ...
                    share_to_groups(result["id"], caption, image, post_type)
                return result
