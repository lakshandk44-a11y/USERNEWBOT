...
# ================= MULTI-FEED RSS (NEW) =================
# Rotating across 4 major feeds = more varied, fresh content every cycle.
# Falls back gracefully if any individual feed fails.
EXTRA_RSS_FEEDS = [
...
# ================= ENGAGEMENT CTA (UPGRADED) =================
# Facebook algorithm weights: Comments (highest) > Reactions > Saves > Shares
# These CTAs are designed to trigger SPECIFIC, low-friction responses that
# stack multiple signals simultaneously. Rotated so no post looks the same.
ENGAGEMENT_CTAS = [
    # Comment triggers (highest weight) — specific = more replies
    "👇 Type YES if you agree — NO if you don't.",
    "👇 Drop 🔥 if this shocked you.",
    "👇 Comment 'I knew it' if you already knew this — or 'No way!' if you didn't.",
    "👇 What's your take? Type 1 if YES, 2 if NO.",
    "👇 Agree or strongly disagree? Tell us in ONE word.",
    "👇 Tag someone who needs to see this right now.",
    "👇 Drop your opinion — we're reading every comment.",

    # Save triggers (underused but powerful signal)
    "📌 SAVE this post — you'll want to come back to it.",
    "📌 Save + Share so your friends see this too.",

    # Share triggers
    "🔁 Share this if you think more people need to know.",
    "🔁 Share to your Story — this is too important to scroll past.",

    # Reaction + comment stacked
    "❤️ React if this matters to you — 💬 comment WHY below.",
    "😮 React if this surprised you — tell us what you think 👇",
]
...
# ================= FOLLOW/SHARE REMINDER (UPGRADED) =================
FOLLOW_REMINDERS = [
    "🔔 Follow this page — we post what mainstream media won't.",
    "👉 Follow for daily updates that actually matter.",
    "🔔 Turn on notifications so you never miss a story.",
    "❤️ Like & Share — help us reach more people!",
    "👥 Share this with someone who needs to see it.",
]
FOLLOW_REMINDER_CHANCE = 0.30  # roughly 1 in 4 posts
...
def pick_hashtags_smart(context_text, n=5):
    """Generate topic-specific hashtags via Gemini for better reach targeting.
    Mix of broad popular + specific niche tags = better algorithm discovery.
    Falls back to static pool if Gemini fails."""
    try:
        text = call_gemini(
            f"Give me {n} Facebook hashtags for this topic: {context_text[:200]}\n"
            f"Mix: 1-2 very broad popular tags (#BreakingNews #WorldNews etc) + "
            f"2-3 specific niche tags relevant to the exact topic.\n"
            f"Return ONLY the hashtags with # prefix separated by spaces. Nothing else.",
            retries=1, label="hashtags_smart"
        )
        if text:
            tags = [w if w.startswith("#") else f"#{w}" for w in text.strip().split() if w.strip("#")]
            tags = [t.strip(".,!?") for t in tags if len(t) > 2]
            if len(tags) >= 2:
                return " ".join(tags[:n])
    except Exception:
        pass
...
QUOTE_SLOTS = [(9, 0)]
FACT_SLOTS = [(19, 30)]
POLL_SLOTS = [(12, 0)]     # NEW: daily reaction poll at noon — high engagement window
# Two daily Reel windows in US Eastern Time. Each day the exact minute is
# randomized inside the window so the page is not locked to the same minute.
VIDEO_SLOT_WINDOWS_ET = [(11, 0, 14, 0), (18, 30, 22, 0)]
VIDEO_SLOTS = [(12, 0), (20, 0)]  # fallback/base values; daily windows below override them
...
def _random_video_slots_for_today():
    """Pick two random US-Eastern Reel times. When enough historical video
    engagement exists, bias the hour toward the better-performing hour while
    keeping randomness inside the two broad audience windows."""
    base_hours = [(11, 14), (18, 22)]
    # weekly_history is loaded later but already exists by the time this helper
    # is called at runtime. Old state files without video/type fields are safe.
    scores = {}
    try:
        for day in weekly_history[-7:]:
            for post in day.get("posts", []):
                if post.get("type") != "video":
                    continue
                hour = int(post.get("hour", -1))
                score = float(post.get("engagement", 0))
                if 0 <= hour <= 23:
                    scores[hour] = scores.get(hour, 0.0) + max(score, 0.1)
    except Exception:
        scores = {}

    slots = []
    for start_h, end_h in base_hours:
        candidates = list(range(start_h, end_h + 1))
        weights = [1.0 + scores.get(h, 0.0) for h in candidates]
        chosen_h = random.choices(candidates, weights=weights, k=1)[0]
        # Random minute inside the selected hour. For the evening window, keep
        # 22:xx inside the requested 18:30-22:00 range.
        if chosen_h == 18 and start_h == 18:
            minute = random.randint(30, 59)
        elif chosen_h == 22 and end_h == 22:
            minute = 0
        else:
            minute = random.randint(0, 59)
        slots.append((chosen_h, minute))
    return slots

_jittered_video_slots = _random_video_slots_for_today()
...
def register_telegram_commands():
    """Register all bot commands with Telegram so they appear in the
    in-app menu (tap '/' to see the list). Called once on startup."""
    ...
    try:
        ...
        if r.json().get("ok"):
            ...
            log(f"⚠️ Telegram setMyCommands failed: {r.text[:200]}")
    except Exception as e:
...
def get_news():
    ...
    try:
        if time.time() - news_last_fetch > 300:
            all_entries = []
            seen_titles = set()
            # Pull from all feeds — primary first, then extras
            for feed_url in [RSS_FEED_URL] + EXTRA_RSS_FEEDS:
                try:
                    ...
                    pass  # one feed failing never kills the whole function
            if all_entries:
                random.shuffle(all_entries)   # mix sources so not all BBC or all Google
                news_cache = all_entries[:60]  # keep up to 60 stories across all feeds
...
def ai_generate(title, desc):
    text = call_gemini(f"""
You are a viral Facebook content strategist. Create a scroll-stopping caption
for this news story that maximizes comments, reactions, and shares.
...
1. HOOK (1 line, max 10 words): Create a curiosity gap or bold statement.
   First 3 words are critical — shown before "See more".
2. BLANK LINE
3. STORY (2-4 sentences): Explain what happened. Specific, clear, factual.
4. BLANK LINE
5. EMOTION LINE (1-2 sentences): Why should people CARE? Personal, relatable.

For image_prompt: cinematic photojournalism, specific to this story, no text.

For hashtags: 5 hashtags — mix 2 broad (#BreakingNews #WorldNews) +
3 specific to this exact topic. Include # prefix.

Return ONLY JSON:
{{"caption":"[hook]\\n\\n[story]\\n\\n[emotion line]","image_prompt":"...","hashtags":"#Tag1 #Tag2 #Tag3 #Tag4 #Tag5"}}
""", label="ai_generate")
    ...
    if text:
        try:
            ...
            caption = (
                ...
                f"{'─' * 24}\n"
                f"{pick_engagement_cta()}\n"
                f"🔁 Share if this sums it up perfectly.\n\n"
                f"{hashtags}"
            )
...
""", label="holiday_generate")
    ...
    if text:
        try:
            ...
            caption = (
                f"{result['caption']}\n\n"
                f"{'─' * 24}\n"
                f"❤️ React to spread the love!\n"
                f"🔁 Share so your friends see this too.\n\n"
                f"#{''.join(holiday_title.split())} #Holiday #Celebration #USA #Viral"
            )
            result["caption"] = apply_monetization(caption)
            return result
        except Exception as e:
    ...
    return {
        "caption": apply_monetization(
            ...
            f"Tag someone who makes this day special 👇\n\n"
            f"#{''.join(holiday_title.split())} #Holiday #Celebration"
        ),
        "image_prompt": (
...
- Or create social sharing urge: "Tag someone who needs to read this today."
- Or create reflection: "Read this twice. Let it sink in."
...
""", label="quote_generate")
    ...
    if not context:
        context = "Tag someone who needs to read this today. 👇"
    if not image_prompt:
        image_prompt = (
    ...
    # Algorithm-optimized caption: hook context + quote + stacked CTAs
    caption = (
        f"✨ {context}\n\n"
        f"💭 \"{quote_text}\"\n\n"
        f"{'─' * 24}\n"
        f"👇 Does this resonate? Drop 🙌 if YES.\n"
        f"📌 Save this — read it again when you need it.\n"
        f"🔁 Share with someone who needs this right now.\n\n"
        f"{pick_hashtags()}"
    )
    return {
...
Hook techniques: "This was hidden for 50 years...", "99%% of people don't know...",
"Scientists just confirmed what we suspected...", "This will change how you see..."
...
""", label="fact_generate")
    ...
    # Shock-hook + fact + save CTA (saves = strong Facebook quality signal)
    caption = (
        f"🤯 {hook}\n\n"
        f"{fact_text}\n\n"
        f"{'─' * 24}\n"
        f"{pick_engagement_cta()}\n"
        f"📌 Save this — share it next time someone says 'I knew that.'\n\n"
        f"{pick_hashtags()}"
    )
    return {
        "caption": apply_monetization(caption),
...
- tag_line: Personalized tag prompt, e.g. "Tag someone who would definitely pick 2️⃣" — max 10 words
- results_line: "We'll share results at [time]!" — creates return visits. Use a time 6-8 hours from noon.
- hashtags: 4-6 trending + topic-relevant hashtags (no generic #DailyVote)
...
""", label="poll_generate")
    ...
    # Structure: Hook → tension build → numbered vote (max comments) →
    # reaction ask → tag → share → results promise → hashtags
    date_label = now().strftime("%B %d").upper()
    caption = (
        # 🔴 LINE 1: Curiosity hook — first thing visible before "See more"
        f"🔴 {hook}\n\n"

        # Subhook builds tension
        f"{subhook}\n\n"

        # Divider — makes post feel structured/premium in feed
        f"{'━' * 28}\n\n"

        # The question — centered visually
        f"           🗳️ {q}\n\n"

        # Numbered voting — single keystroke = max comment volume
        f"  1️⃣  {op1}\n"
        f"  2️⃣  {op2}\n\n"

        f"{'━' * 28}\n\n"

        # Stacked engagement asks (each one = separate algorithm signal)
        f"👇 DROP your number in the comments — 1️⃣ or 2️⃣\n"
        f"❤️ React if you're a SOLID 1️⃣\n"
        f"😂 React if you're a SOLID 2️⃣\n\n"

        # Tag prompt — organic reach to non-followers
        f"📲 {tag_line}\n"

        # Share ask — viral coefficient
        f"🔁 Share to your Story — see what your friends pick!\n\n"

        # Results promise — drives return visits (algorithm quality signal)
        f"👀 {results_line}\n\n"

        # Hashtags
        f"{hashtags} #Poll #React #{now().strftime('%B')}2026"
    )
    ...
    try:
        ...
        for post in data.get("data", []):
            for comment in post.get("comments", {}).get("data", []):
                # Skip Page's own top-level comments
                if comment.get("from", {}).get("id", "") != FB_PAGE_ID:
                    comments.append(comment)
                # Also collect replies to this comment (nested)
...
def post_self_comment(post_id, topic):
    """Post an engaging question as a Page comment on our own post right after
    publishing. For poll posts, seeds the first numbered vote to show others
    the format and kickstart the comment thread immediately."""

    # Poll posts get a special seed comment that models the 1️⃣/2️⃣ voting format
    # This is critical — seeing the first comment in the format makes every
    # subsequent reader follow the same pattern (comment thread = algorithm gold)
    if topic == "poll":
        seed_comment = (
            "We'll go first 👀  Drop 1️⃣ or 2️⃣ below — "
            "we're reading every comment! 🔥"
        )
        try:
            url = f"https://graph.facebook.com/v20.0/{post_id}/comments"
            resp = requests.post(url, data={
                "message": seed_comment,
                "access_token": FB_ACCESS_TOKEN
            }, timeout=REQUEST_TIMEOUT).json()
            if "error" not in resp:
                log(f"✅ Poll seed comment posted on {post_id}")
            else:
                log(f"⚠️ Poll seed comment failed on {post_id}: {resp['error']}")
        except Exception as e:
            log(f"⚠️ post_self_comment (poll) exception: {e}")
        return

    # All other post types — pick or generate a type-specific seed comment
    # that models the expected engagement behavior for that content type.
    TYPE_SEED_COMMENTS = {
        "quote": [
            "This one hit us differently today. Does it resonate with you? 👇",
            "Read this twice. Which part speaks to you most? 💬",
            "Tag someone who needs this right now. 👇",
        ],
        "fact": [
            "Did THIS surprise you? Drop 🤯 or 'I knew it' below 👇",
            "Which part shocked you most? Tell us 👇",
            "Save this — you'll want to share it later. Which fact got you? 💬",
        ],
        "scenic": [
            "Would you visit this place? Drop 1️⃣ for YES, 2️⃣ for maybe someday 👇",
            "Tag someone you'd want to go here with 📍👇",
            "Does this make you want to travel? Tell us where you'd go instead 🌍",
        ],
        "holiday": [
            "How are you celebrating today? Tell us below 🎉👇",
            "Wishing everyone a wonderful day! Drop ❤️ to spread the love 👇",
        ],
        "news": [
            "What's YOUR take on this? Drop your opinion below 👇",
            "Does this surprise you? React + comment your thoughts 💬",
            "This is big. Tag someone who needs to see this 👇",
        ],
    }

    seed_options = TYPE_SEED_COMMENTS.get(topic, [])
    if seed_options:
        question = random.choice(seed_options)
    else:
        # Fallback: generate via Gemini for unknown post types
        text = call_gemini(
            f"Write ONE short, punchy question (max 12 words) to post as a comment on a "
            f"Facebook post about: {topic}\n"
            f"Goal: get followers to reply — make it irresistible and opinionated.\n"
            f"Reply ONLY with the question text, nothing else, no quotes.",
            retries=2, label="self_comment"
        )
        if not text:
            return
    ...
    try:
        ...
        if "error" not in resp:
            log(f"✅ Self-comment posted on {post_id}: {question}")
        else:
            log(f"⚠️ Self-comment failed on {post_id}: {resp['error']}")
    except Exception as e:
        log(f"⚠️ post_self_comment exception: {e}")
...
def maybe_revive_post(post_id, post_type, posted_at_iso):
    """Called once per scheduler loop for each recent post. Revives once only."""
    if post_id in _revive_checked_ids:
        return
    try:
        posted_at = datetime.fromisoformat(posted_at_iso)
    except Exception:
        _revive_checked_ids.add(post_id)
        return

    elapsed = (datetime.now(pytz.utc) - posted_at.replace(tzinfo=pytz.utc)
               if posted_at.tzinfo is None
               else (datetime.now(pytz.utc) - posted_at)).total_seconds()

    if elapsed < REVIVE_DELAY_SECONDS:
        return   # too soon — check again next loop

    _revive_checked_ids.add(post_id)   # mark now so we don't re-check even on failure

    eng = get_post_engagement(post_id) or {}
    total = eng.get("likes", 0) + eng.get("comments", 0) + eng.get("shares", 0)
    if total >= REVIVE_LOW_THRESHOLD:
        return   # doing fine — no revive needed

    # Generate and post the revival comment
    revive_text = call_gemini(f"""
Write ONE short, engaging revival comment (max 15 words) to post on a
Facebook {post_type} post that got very low engagement. Goal: re-spark
interest and get people to react or comment. Be punchy and direct.
Example styles: "Drop a ❤️ if you agree!", "Tell us what YOU think 👇",
"This one deserves more attention — share it!"
Reply ONLY with the comment text, nothing else.
""", retries=2, label="revive_comment")

    if not revive_text:
        return

    comment = revive_text.strip().strip('"').strip("'")
    try:
        url = f"https://graph.facebook.com/v20.0/{post_id}/comments"
        resp = requests.post(url, data={
            "message": comment,
        ...
        if "error" not in resp:
            log(f"🔁 Revive comment posted on {post_id} (engagement was low: "
                f"👍{eng.get('likes',0)} 💬{eng.get('comments',0)}): {comment}")
        else:
            log(f"⚠️ Revive comment failed on {post_id}: {resp['error']}")
    except Exception as e:
        log(f"⚠️ maybe_revive_post exception: {e}")


def process_auto_replies():
    # Rate limit: max 4 replies per scheduler loop (every 20 sec).
    # Replying to 100 comments in one burst = Facebook rate-limit / token ban risk.
    # 4/loop × ~180 loops/hour = up to 720 replies/hour — more than enough for
    # any organic page, while staying well inside Graph API limits.
    MAX_REPLIES_PER_LOOP = 4
    replies_sent = 0
...
VIDEO_TOPICS = [
    "fun fact", "quick history moment", "life hack", "mind-blowing science fact",
    "quote & motivation", "did-you-know trivia", "shocking statistic",
]

US_NEWS_KEYWORDS = {
    "us": 4, "u.s.": 5, "united states": 5, "america": 5, "american": 4,
    "white house": 6, "washington": 5, "congress": 5, "senate": 5, "house": 2,
    "president": 4, "trump": 5, "biden": 4, "supreme court": 6,
    "federal": 4, "fbi": 4, "cia": 3, "nasa": 3, "pentagon": 5,
    "california": 4, "texas": 4, "florida": 4, "new york": 4,
    "wall street": 5, "nasdaq": 4, "dow": 3, "fed": 3, "inflation": 3,
    "hurricane": 3, "wildfire": 3, "tornado": 3, "earthquake": 3,
}


def _select_video_topic():
    """Prefer fresh, US-relevant news while preserving all legacy video topics."""
    try:
        news = get_news()
        scored = []
        for item in news:
            ...
            score = sum(weight for key, weight in US_NEWS_KEYWORDS.items() if key in text)
            if score <= 0:
                continue
            # Prefer recent-looking news titles and avoid topics already used today.
            key = re.sub(r"[^a-z0-9 ]", "", title.lower()).strip()[:100]
            if key in seen_video_topics:
                continue
            score += min(len(title), 120) / 120.0
            scored.append((score, item))
        if scored:
            scored.sort(key=lambda x: x[0], reverse=True)
            # Small randomness among the top stories prevents the exact same story
            # from winning every cycle while still favoring high-relevance news.
            top = scored[:min(5, len(scored))]
            return {"type": "news", **random.choice(top)[1]}
    except Exception as e:
        log(f"⚠️ _select_video_topic news selection failed: {e}")

    # Legacy topics remain available as a fallback, exactly as before.
    return {"type": "evergreen", "topic": random.choice(VIDEO_TOPICS)}


def generate_video_script(topic_hint=None):
    """Generate a short original Reel from current US news or a legacy topic."""
    selected = topic_hint or _select_video_topic()
    if isinstance(selected, dict) and selected.get("type") == "news":
        title = selected.get("title", "")
        desc = selected.get("desc", title)
        prompt = f"CURRENT US NEWS STORY\nHeadline: {title}\nDetails: {desc}"
        mode = "US_NEWS"
    elif isinstance(selected, dict):
        prompt = selected.get("topic", "fun fact")
        mode = "EVERGREEN"
    else:
        prompt = str(selected)
        mode = "EVERGREEN"
    ...
    text = call_gemini(f"""
You are an expert US short-form news/Reels editor. Create an original,
accurate, non-misleading 25-40 second vertical Facebook Reel.
...
Rules:
- Preserve factual meaning. Never invent names, numbers, quotes, or events.
- If MODE is US_NEWS, focus on what happened, why it matters to Americans,
  and what is known next. Do not present rumors as facts.
- If MODE is EVERGREEN, make it genuinely useful/interesting and clearly
  distinguish facts from opinions.
- 1 hook + 4 content slides = 5 slides total.
- Hook: 6-12 words, immediate curiosity without clickbait deception.
- Each content slide: 8-18 spoken words, concise and natural.
- Total narration: roughly 30-38 seconds.
- Each image prompt must be cinematic, vertical-friendly, photorealistic,
  specific to the narration, and contain NO text, logos, watermarks, or fake
  screenshots of news articles.
- Write for a US audience in natural US English.
- Caption: factual hook + short context + natural discussion question +
  3-5 topic-specific hashtags. Avoid spam tags and engagement bait such as
  "comment YES", "share this NOW", or fake urgency.
- If this is a news story, include a concise source label in the caption when
  a publisher/source is available from the input.
- topic_key must be unique and based on the story/topic.
...
""", label="video_script")
    ...
    try:
        ...
        result["mode"] = mode
        if mode == "US_NEWS":
            result["source_headline"] = str(selected.get("title", ""))[:200]
        return result
    except Exception as e:
...
def _write_karaoke_ass(narration, duration, ass_path):
    """Create a lightweight word-synced ASS subtitle using proportional timing."""
    words = re.findall(r"\S+", narration or "")
    if not words:
        return False
    weights = [max(len(re.sub(r"[^A-Za-z0-9]", "", w)), 1) for w in words]
    total = float(sum(weights))
    events = []
    t = 0.0
    for word, weight in zip(words, weights):
        d = max(0.08, duration * weight / total)
        end = min(duration, t + d)
        events.append((t, end, word))
        t = end
    def ts(sec):
        sec = max(0.0, sec)
    ...
    try:
        with open(ass_path, 'w', encoding='utf-8') as f:
            ...
            f.write("Style: Default,DejaVu Sans,52,&H00FFFFFF,&H0000FFFF,&H00000000,&H80000000,1,0,1,3,1,2,70,70,260,1\n\n")
            f.write("[Events]\nFormat: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n")
            # Each word gets its own short event. Current word is yellow; surrounding
            # words remain white. This is robust and avoids needing per-word TTS calls.
            for i, (a, b, word) in enumerate(events):
                f.write(f"Dialogue: 0,{ts(a)},{ts(b)},Default,,0,0,0,,{{\\c&H00FFFF&}}{ass_escape(word)}\\N\n")
        return True
    except Exception as e:
        log(f"⚠️ karaoke subtitle generation failed: {e}")
        return False


def _make_generated_audio(duration, session_dir):
    """Generate subtle royalty-free ambient bed + short transition SFX with FFmpeg."""
    music = os.path.join(session_dir, "ambient.m4a")
    sfx = os.path.join(session_dir, "whoosh.wav")
    try:
        # Two quiet tones, shaped by low-pass + volume + fades, make a neutral
        # ambient bed without downloading copyrighted music onto the VPS.
        r = subprocess.run([
            "ffmpeg", "-y", "-f", "lavfi", "-i",
            f"sine=frequency=196:duration={duration}", "-f", "lavfi", "-i",
            f"sine=frequency=293.66:duration={duration}", "-filter_complex",
            "[0:a]volume=0.025,lowpass=f=900[a0];[1:a]volume=0.015,lowpass=f=1200[a1];"
            "[a0][a1]amix=inputs=2:duration=longest,afade=t=in:st=0:d=1,afade=t=out:st="
            f"{max(duration-1,0.5)}:d=1[out]", "-map", "[out]", "-c:a", "aac", music
        ], capture_output=True, timeout=30)
        # Short rising/falling whoosh; repeated at low volume around transitions.
        r2 = subprocess.run([
            "ffmpeg", "-y", "-f", "lavfi", "-i", "sine=frequency=900:duration=0.35",
            "-af", "afade=t=in:st=0:d=0.03,afade=t=out:st=0.18:d=0.17,volume=0.06",
            "-c:a", "pcm_s16le", sfx
        ], capture_output=True, timeout=15)
        return (os.path.exists(music) and os.path.getsize(music) > 1000,
                os.path.exists(sfx) and os.path.getsize(sfx) > 1000)
    except Exception as e:
        log(f"⚠️ generated audio bed failed: {e}")
        return False, False


def build_video_reel(slides, out_path):
    """Build a safe 1080x1920 Reel with camera motion, transitions, karaoke captions,
    voiceover and a very subtle generated audio bed. Falls back to simpler assembly
    if an optional FFmpeg filter fails, so other bot functions are unaffected."""
    if not FFMPEG_AVAILABLE:
        log("⚠️ build_video_reel skipped: ffmpeg not installed on this server.")
        return False
    session_dir = os.path.join(VIDEO_TEMP_DIR, f"{int(time.time())}_{random.randint(1000,9999)}")
    ...
    try:
        ...
        has_voice = _generate_voiceover(full_narration, audio_path)
        total_duration = _get_media_duration(audio_path) if has_voice else len(slides) * 4.5
        total_duration = max(8.0, min(total_duration, 55.0))
        per_slide = max(total_duration / len(slides), 2.2)
        total_duration = per_slide * len(slides)

        clip_paths = []
        for i, (img_path, slide) in enumerate(zip(img_paths, slides)):
            clip_path = os.path.join(session_dir, f"clip_{i}.mp4")
            ass_path = os.path.join(session_dir, f"cap_{i}.ass")
            frames = max(1, int(per_slide * 25))
            mode = i % 4
            if mode == 0:
                zoom = "min(zoom+0.0014,1.22)"; x = "iw/2-(iw/zoom/2)"; y = "ih/2-(ih/zoom/2)"
            elif mode == 1:
                zoom = "min(zoom+0.0011,1.18)"; x = "iw/2-(iw/zoom/2)+sin(on/45)*45"; y = "ih/2-(ih/zoom/2)"
            elif mode == 2:
                zoom = "min(zoom+0.0013,1.20)"; x = "iw/2-(iw/zoom/2)"; y = "ih/2-(ih/zoom/2)+sin(on/50)*35"
            else:
                zoom = "max(1.18,zoom-0.0010)"; x = "iw/2-(iw/zoom/2)"; y = "ih/2-(ih/zoom/2)"
            vf_parts = [
                "scale=1080:1920:force_original_aspect_ratio=increase",
                "crop=1080:1920",
                f"zoompan=z='{zoom}':x='{x}':y='{y}':d={frames}:s=1080x1920:fps=25",
                "eq=contrast=1.04:saturation=1.08:brightness=0.01",
                "vignette=PI/5"
            ]
            if _VIDEO_FONT:
                # ASS is used for word-synced captions; if it cannot be created,
                # the video still gets a readable single-line caption fallback.
                if _write_karaoke_ass(slide.get("narration", ""), per_slide, ass_path):
                    vf_parts.append(f"ass='{ass_path}'")
                else:
                    caption = _escape_ffmpeg_text(slide.get("narration", "")[:100])
                    vf_parts.append(f"drawtext=fontfile={_VIDEO_FONT}:text='{caption}':fontsize=50:fontcolor=white:box=1:boxcolor=black@0.55:boxborderw=20:x=(w-text_w)/2:y=h-360")
            cmd = ["ffmpeg", "-y", "-loop", "1", "-i", img_path, "-vf", ",".join(vf_parts),
                   "-t", str(per_slide), "-an", "-c:v", "libx264", "-preset", "veryfast",
                   "-crf", "20", "-pix_fmt", "yuv420p", "-r", "25", clip_path]
            r = subprocess.run(cmd, capture_output=True, timeout=75)
            if r.returncode != 0 or not os.path.exists(clip_path):
                log(f"⚠️ enhanced ffmpeg slide {i} failed; retrying basic video filter.")
                basic = ["scale=1080:1920:force_original_aspect_ratio=increase","crop=1080:1920",
                         f"zoompan=z='min(zoom+0.0012,1.20)':d={frames}:s=1080x1920:fps=25"]
                if _VIDEO_FONT:
                    cap = _escape_ffmpeg_text(slide.get("narration", "")[:90])
                    basic.append(f"drawtext=fontfile={_VIDEO_FONT}:text='{cap}':fontsize=48:fontcolor=white:box=1:boxcolor=black@0.55:boxborderw=18:x=(w-text_w)/2:y=h-340")
                r = subprocess.run(["ffmpeg","-y","-loop","1","-i",img_path,"-vf",",".join(basic),"-t",str(per_slide),"-an","-c:v","libx264","-pix_fmt","yuv420p","-r","25",clip_path], capture_output=True, timeout=75)
            if r.returncode != 0 or not os.path.exists(clip_path):
                log(f"⚠️ ffmpeg slide {i} failed: {r.stderr.decode(errors='ignore')[-500:]}")
                return False
            clip_paths.append(clip_path)

        # Try cinematic xfade transitions first. If unavailable/unsupported,
        # fall back to concat so an optional visual enhancement can never break posting.
        ...
        if len(clip_paths) > 1:
            ...
            filters = []
            current = "[0:v]"
            offset = per_slide - 0.35
            for i in range(1, len(clip_paths)):
                out = f"[v{i}]"
                filters.append(f"{current}[{i}:v]xfade=transition={['fade','smoothleft','wipeleft','circleopen'][i%4]}:duration=0.35:offset={offset:.3f}{out}")
                current = out
                offset += per_slide - 0.35
            r = subprocess.run(["ffmpeg","-y",*inputs,"-filter_complex",";".join(filters),"-map",current,"-an","-c:v","libx264","-preset","veryfast","-crf","20","-pix_fmt","yuv420p","-r","25",silent_video_path], capture_output=True, timeout=120)
            xfade_ok = r.returncode == 0 and os.path.exists(silent_video_path)
        if not xfade_ok:
            concat_list_path = os.path.join(session_dir, "concat.txt")
            with open(concat_list_path, "w") as f:
                for cp in clip_paths:
                    f.write("file '" + cp.replace("'", "'\''") + "'\n")
            r = subprocess.run(["ffmpeg","-y","-f","concat","-safe","0","-i",concat_list_path,"-c","copy",silent_video_path], capture_output=True, timeout=90)
            if r.returncode != 0 or not os.path.exists(silent_video_path):
                log(f"⚠️ ffmpeg concat failed: {r.stderr.decode(errors='ignore')[:500]}")
                return False

        music_ok, sfx_ok = _make_generated_audio(total_duration, session_dir)
        final_audio = os.path.join(session_dir, "mix.m4a")
        audio_inputs = []
        filters = []
        if has_voice:
            audio_inputs += ["-i", audio_path]
            filters.append("[0:a]volume=1.0[voice]")
        idx = 1 if has_voice else 0
        if music_ok:
            audio_inputs += ["-i", os.path.join(session_dir,"ambient.m4a")]
            filters.append(f"[{idx}:a]volume=0.22[music]")
            idx += 1
        if sfx_ok and len(clip_paths) > 1:
            # A single subtle SFX stream is enough; loop it and keep it very quiet.
            audio_inputs += ["-stream_loop", str(len(clip_paths)-2), "-i", os.path.join(session_dir,"whoosh.wav")]
            filters.append(f"[{idx}:a]volume=0.12,atrim=duration={total_duration}[sfx]")
            idx += 1
        labels = []
        if has_voice: labels.append("[voice]")
        if music_ok: labels.append("[music]")
        if sfx_ok and len(clip_paths) > 1: labels.append("[sfx]")
        if labels:
            filters.append("".join(labels) + f"amix=inputs={len(labels)}:duration=longest:dropout_transition=2[aout]")
            r = subprocess.run(["ffmpeg","-y","-i",silent_video_path,*audio_inputs,"-filter_complex",";".join(filters),"-map","0:v","-map","[aout]","-c:v","copy","-c:a","aac","-b:a","192k","-shortest","-movflags","+faststart",out_path], capture_output=True, timeout=120)
        else:
            r = subprocess.run(["ffmpeg","-y","-i",silent_video_path,"-c:v","copy","-an","-movflags","+faststart",out_path], capture_output=True, timeout=90)
        if r.returncode != 0 or not os.path.exists(out_path):
            log(f"⚠️ final audio/video mux failed: {r.stderr.decode(errors='ignore')[-700:]}")
            return False
        return True
    except Exception as e:
...
def post_video_reel(video_path, caption):
    """
...
def generate_and_post_video(topic_hint=None):
    ...
    if not (FFMPEG_AVAILABLE and EDGE_TTS_AVAILABLE):
        missing = []
        if not FFMPEG_AVAILABLE: missing.append("ffmpeg (install with your OS package manager)")
        if not EDGE_TTS_AVAILABLE: missing.append("edge-tts (install in the bot virtualenv)")
    ...
    out_path = os.path.join(VIDEO_TEMP_DIR, f"reel_{int(time.time())}.mp4")

    if not build_video_reel(slides, out_path):
        log("❌ generate_and_post_video: video build failed.")
        return False

    caption = script.get("caption", "🎬 Daily video")
    # Normalize hashtags and add source transparency for news videos without
    # disturbing the existing caption/monetization system.
    if script.get("mode") == "US_NEWS":
        tags = pick_hashtags_smart(script.get("source_headline", "US news"), n=5)
        if tags and tags not in caption:
            caption = caption.rstrip() + "\n\n" + tags
    caption = apply_monetization(caption)
    result = post_video_reel(out_path, caption)
        ...
        try:
            ...
            if "error" not in result:
                ...
                if "id" in result:
                    ...
                    post_self_comment(result["id"], alt_text or post_type)
...
def daily_summary():
    ...
    for p in daily_posted_posts:
        ...
        post_details.append({"hour": p.get("hour"), "type": p.get("type"), "engagement": score})
...
def scheduler():
    ...
    while True:
        try:
            ...
            # ===== ENGAGEMENT REVIVE (NEW) =====
            for p in list(daily_posted_posts):
                maybe_revive_post(p["id"], p["type"], p["time"])
