...
def _jitter_slots(slots):
    """Apply a random 0-4 min offset to each (h, m) slot. Safe across hour boundary."""
    result = []
    for (h, m) in slots:
...
def post_self_comment(post_id, topic):
    ...
    publishing. This is the single biggest organic reach lever available without
    paid promotion — do NOT remove or disable."""
    text = call_gemini(f"""
Write ONE short, punchy question (max 12 words) to post as a comment on a
Facebook post about: {topic}
Goal: get followers to reply — make it irresistible and opinionated.
Examples of good questions: "Would you support this? 👇",
"Hot take or common sense? Tell us!", "Do you agree or disagree? 🤔"
Reply ONLY with the question text, nothing else, no quotes.
...
def scheduler():
    ...
    global _jittered_quote_slots, _jittered_fact_slots
