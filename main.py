...
def _build_karaoke_ass(word_timings, ass_path, words_per_line=5, max_line_chars=32):
    """
    ...
    # Group words into short, phone-readable lines.
    lines, current, current_len = [], [], 0
    for w in word_timings:
        wl = len(w["word"]) + 1
        if current and (len(current) >= words_per_line or current_len + wl > max_line_chars):
    ...
    header = (
        ...
        f"Style: Caption,{family},66,{BASE},&H000000FF,&H00101010,&H88000000,"
        "1,0,0,0,100,100,0.5,0,1,4,2,2,60,60,360,1\n\n"
        "[Events]\n"
    ...
    for line_words in lines:
        ...
        for idx, active in enumerate(line_words):
            start = active["start"]
            end = line_words[idx + 1]["start"] if idx + 1 < len(line_words) else line_end + 0.18
            if end <= start:
                end = start + 0.10
            parts = []
        ...
        _ = line_start  # kept for clarity/debuggability; not otherwise used
...
def build_video_reel(slides, out_path):
    ...
    try:
        ...
        full_narration = ". ".join(
            str(s.get("narration", "")).strip().rstrip(".!?") + "."
            for s in slides
        )
        ...
        use_karaoke_captions = bool(_VIDEO_FONT) and bool(word_timings) and _build_karaoke_ass(word_timings, ass_path)
...
def build_reel_cover(slides, cover_path):
    ...
    try:
        ...
        vf = [
            ...
            "crop=1080:1920",
            "eq=brightness=-0.10:saturation=1.14:contrast=1.10:gamma=1.02",
            "unsharp=5:5:0.4:5:5:0.0",
            "vignette=PI/5",
        ]
        if _VIDEO_FONT:
            # The dark title box is only drawn together with its text — this
            # previously ran unconditionally, so a server with no matching
            # font just showed a plain black rectangle with nothing on it.
            vf.append("drawbox=x=45:y=610:w=990:h=730:color=black@0.45:t=fill")
            vf.extend([
                f"drawtext=fontfile={_VIDEO_FONT}:text='YOU NEED TO SEE THIS':fontsize=34:fontcolor=white@0.92:x=(w-text_w)/2:y=735:borderw=1:bordercolor=black",
                ...
                f"drawtext=fontfile={_VIDEO_FONT}:text='{subtitle}':fontsize=38:fontcolor=white@0.94:x=(w-text_w)/2:y=1080:borderw=1:bordercolor=black",
            ])
        else:
