...
VIDEO_SLOTS = [(19, 0)]    # NEW: daily video Reel — evening, when Reels get algorithm priority
...
_jittered_video_slots = _jitter_slots(VIDEO_SLOTS)   # NEW
...
def maybe_revive_post(post_id, post_type, posted_at_iso):
    ...
    Important: posts that were already present when the process started are
    ignored for revival. This prevents a PM2 restart from immediately posting
    a new Page comment on an old low-engagement post. Posts created after
    startup keep the original revival behavior.
    """
    if post_id in _startup_existing_post_ids:
        return
...
def generate_video_script(topic_hint=None):
    ...
    topic = topic_hint or random.choice(VIDEO_TOPICS)
    text = call_gemini(f"""
...
- caption: a short Facebook caption for the post itself (with hook + CTA + hashtags)
...
def build_video_reel(slides, out_path):
    ...
    try:
        ...
        # Split total narration duration evenly across slides (simple and
        # reliable — avoids needing per-slide TTS calls / sync complexity).
        if has_audio:
            total_duration = _get_media_duration(audio_path)
        else:
            total_duration = len(slides) * 4.0   # fallback: 4s per slide, no voice
        # Facebook Reels require 4-60 seconds — clamp to stay safely inside
        # that range even if the script/voiceover runs unexpectedly long/short.
        ...
        for i, (img_path, slide) in enumerate(zip(img_paths, slides)):
            ...
            frames = int(per_slide * 25)  # 25 fps
