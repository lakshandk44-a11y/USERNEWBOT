...
def save_state():
    try:
        ...
        with STATE_SAVE_LOCK:
            temp_state = f"{STATE_FILE}.{os.getpid()}.tmp"
...
def _stop_live_camera(reason="Stopped by Telegram"):
    ...
    with live_state_lock:
        was_active = bool(live_enabled or live_process is not None or live_video_id)
        live_enabled = False
        proc = live_process
...
def _video_env_int(name, default, minimum=None, maximum=None):
    try:
...
def _video_env_float(name, default, minimum=None, maximum=None):
    import math
    try:
...
# This section contains the complete video-only production pipeline.
# Nothing outside this section is required by the renderer, so the existing
# bot scheduler, post types, analytics and other automation remain untouched.
#
# Pipeline:
#   factual script -> scene-specific image generation -> visual deduplication
#   -> Edge-TTS narration -> word-timed ASS captions -> cinematic render
#   -> professional cover -> Facebook Reel upload

VIDEO_TEMP_DIR = os.getenv("VIDEO_TEMP_DIR", "/tmp/fb_bot_video")
VIDEO_VOICE = os.getenv("VIDEO_VOICE", "en-US-AriaNeural")
VIDEO_VOICE_RATE = os.getenv("VIDEO_VOICE_RATE", "-3%")
VIDEO_VOICE_PITCH = os.getenv("VIDEO_VOICE_PITCH", "+0Hz")
VIDEO_MUSIC_FILE = os.getenv("VIDEO_MUSIC_FILE", "").strip()

VIDEO_MUSIC_VOLUME = _video_env_float("VIDEO_MUSIC_VOLUME", 0.055, 0.0, 0.12)

VIDEO_WIDTH = 1080
VIDEO_HEIGHT = 1920
VIDEO_FPS = 30
VIDEO_CRF = str(_video_env_int("VIDEO_CRF", 18, 14, 28))
VIDEO_PRESET = os.getenv("VIDEO_PRESET", "medium").strip().lower()
if VIDEO_PRESET not in {"ultrafast", "superfast", "veryfast", "faster", "fast", "medium", "slow", "slower", "veryslow"}:
    VIDEO_PRESET = "medium"
VIDEO_MIN_DURATION = 8.0
VIDEO_MAX_DURATION = 55.0
VIDEO_TRANSITION_SECONDS = 0.30
VIDEO_CAPTION_FONT_SIZE = _video_env_int("VIDEO_CAPTION_FONT_SIZE", 54, 28, 90)
VIDEO_CAPTION_MARGIN_V = _video_env_int("VIDEO_CAPTION_MARGIN_V", 365, 260, 520)
VIDEO_COVER_QUALITY = _video_env_int("VIDEO_COVER_QUALITY", 2, 1, 8)
VIDEO_MAX_IMAGE_RETRIES = _video_env_int("VIDEO_MAX_IMAGE_RETRIES", 2, 1, 5)
VIDEO_IMAGE_MEMORY_LIMIT = _video_env_int("VIDEO_IMAGE_MEMORY_LIMIT", 120, 20, 500)
VIDEO_IMAGE_SIMILARITY_THRESHOLD = _video_env_float("VIDEO_IMAGE_SIMILARITY_THRESHOLD", 0.105, 0.01, 0.50)
VIDEO_MEMORY_FILE = os.getenv(
    "VIDEO_IMAGE_MEMORY_FILE",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "video_image_memory.json"),
)
VIDEO_RENDER_TIMEOUT = _video_env_int("VIDEO_RENDER_TIMEOUT", 300, 120, 1800)
VIDEO_REQUIRE_VOICE = os.getenv("VIDEO_REQUIRE_VOICE", "true").strip().lower() in {"1", "true", "yes", "on"}
VIDEO_MEMORY_LOCK = Lock()
...
VIDEO_SCENE_ROLES = [
    {
        "name": "HOOK",
        "shot": "cinematic close-up or striking subject detail",
        "camera": "shallow depth of field, strong foreground separation, deliberate composition",
        "motion": "slow push-in or subtle lateral camera movement",
    },
    {
        "name": "WHAT HAPPENED",
        "shot": "medium documentary action shot showing the central event or object in context",
        "camera": "natural eye-level perspective, believable human scale, decisive moment",
        "motion": "gentle left-to-right visual flow",
    },
    {
        "name": "WHY IT MATTERS",
        "shot": "wide establishing scene showing location, environment and consequences",
        "camera": "layered foreground, subject and background depth, cinematic perspective",
        "motion": "slow pull-back or controlled reveal",
    },
    {
        "name": "IMPORTANT DETAIL",
        "shot": "macro or tight detail shot of the most informative physical clue, object or action",
        "camera": "high detail, realistic texture, controlled focus falloff",
        "motion": "slow micro push-in",
    },
    {
        "name": "TAKEAWAY",
        "shot": "human-centered contextual scene or memorable final wide composition",
        "camera": "clean editorial framing with visual breathing room for captions",
        "motion": "slow cinematic drift toward the focal point",
    },
]

VIDEO_CATEGORIES = [
    "breaking US news explained",
    "American history mysteries and forgotten moments",
    "US technology and AI breakthroughs",
    "space and astronomy discoveries",
    "science discoveries that affect everyday life",
    "American money, business and consumer trends",
    "health, fitness and human-body science facts",
    "weather, extreme weather and climate stories in the US",
    "American places, landmarks and hidden facts",
    "strange but verified US facts",
    "engineering and inventions that changed America",
    "everyday psychology and human behavior",
    "wildlife and nature stories from the US",
    "food science and surprising American food facts",
    "crime, mystery and historical unsolved stories (factual, non-graphic)",
    "future technology and how it may change daily life",
    "quick history: one event that changed America",
    "did-you-know: useful facts with a clear explanation",
]

VIDEO_TOPICS = VIDEO_CATEGORIES



def _video_image_memory_load():
    """Load the small persistent visual-memory index used only by Reels."""
    try:
        if not os.path.exists(VIDEO_MEMORY_FILE):
            return []
        with open(VIDEO_MEMORY_FILE, "r", encoding="utf-8") as handle:
            data = json.load(handle)
        if not isinstance(data, list):
            return []
        clean = []
        for item in data[-VIDEO_IMAGE_MEMORY_LIMIT:]:
            if isinstance(item, dict) and isinstance(item.get("fingerprint"), str):
                clean.append(item)
        return clean
    except Exception as e:
        log(f"⚠️ Video image memory load failed: {e}")
        return []


def _video_image_memory_save(memory):
    """Atomically and durably save the bounded visual-memory index."""
    temp_path = f"{VIDEO_MEMORY_FILE}.{os.getpid()}.{random.randint(1000, 999999)}.tmp"
    try:
        os.makedirs(os.path.dirname(VIDEO_MEMORY_FILE) or ".", exist_ok=True)
        bounded = memory[-VIDEO_IMAGE_MEMORY_LIMIT:]
        with VIDEO_MEMORY_LOCK:
            with open(temp_path, "w", encoding="utf-8") as handle:
                json.dump(bounded, handle, ensure_ascii=False, separators=(",", ":"))
                handle.flush()
                os.fsync(handle.fileno())
            os.replace(temp_path, VIDEO_MEMORY_FILE)
        return True
    except Exception as e:
        log(f"⚠️ Video image memory save failed: {e}")
        try:
...
def _video_image_fingerprint(image_bytes):
    """Create a compact perceptual grayscale fingerprint using FFmpeg."""
    if not image_bytes or not FFMPEG_AVAILABLE:
        return None
    try:
        result = subprocess.run(
            [
                ...
                "-i", "pipe:0",
                "-vf", "scale=24:24:flags=bilinear,format=gray",
                "-frames:v", "1",
                "-f", "rawvideo", "pipe:1",
            ],
            input=image_bytes,
        ...
        raw = result.stdout
        if result.returncode != 0 or len(raw) != 576:
            return None
        # Quantize lightly so tiny compression differences do not defeat the
        # duplicate detector while still preserving composition differences.
        return base64.b64encode(bytes((value // 8) * 8 for value in raw)).decode("ascii")
    except Exception as e:
        log(f"⚠️ Video image fingerprint failed: {e}")
        return None


def _video_image_distance(fingerprint_a, fingerprint_b):
    """Return normalized perceptual distance in the range 0..1."""
    if not fingerprint_a or not fingerprint_b:
        return 1.0
    try:
        a = base64.b64decode(fingerprint_a.encode("ascii"), validate=True)
        b = base64.b64decode(fingerprint_b.encode("ascii"), validate=True)
...
def _video_image_is_too_similar(fingerprint, memory, current_fingerprints):
    if not fingerprint:
        return False
    for previous in list(current_fingerprints) + [x.get("fingerprint") for x in memory]:
        if previous and _video_image_distance(fingerprint, previous) < VIDEO_IMAGE_SIMILARITY_THRESHOLD:
            return True
    return False


def _remember_video_image(fingerprint, topic_key, scene_index, memory):
    if not fingerprint:
        return
    memory.append({
        "fingerprint": fingerprint,
        "topic_key": str(topic_key or "video")[:120],
        "scene": int(scene_index) + 1,
        "time": int(time.time()),
    })
    del memory[:-VIDEO_IMAGE_MEMORY_LIMIT]


def _pick_video_topic(video_index=None):
    """Choose a US-relevant topic without creating another network request."""
    try:
        stories = get_news()
    ...
    if video_index is not None:
        return VIDEO_CATEGORIES[int(video_index) % len(VIDEO_CATEGORIES)]
    return random.choice(VIDEO_CATEGORIES)


def generate_video_script(topic_hint=None):
    """Generate a factual five-scene script with deliberately diverse visuals."""
    topic = topic_hint or _pick_video_topic()
    text = call_gemini(f"""
You are the senior editor of a premium US short-form documentary/news Reel.

Create ONE original, factual, easy-to-understand 25-35 second faceless Reel about:
{topic}

The viewer must understand the story without reading the Facebook caption.
Never invent facts, names, numbers, quotes, events, locations or causes. If a
claim is uncertain, use cautious language rather than presenting it as confirmed.

Create exactly 5 scenes:
1) HOOK — the strongest visual and the clearest curiosity-driven statement.
2) WHAT HAPPENED — the central fact or event.
3) WHY IT MATTERS — the real-world consequence or meaning.
4) IMPORTANT DETAIL — one concrete detail, object, place, number or context.
5) TAKEAWAY — a memorable but factual closing point.

Narration rules:
- 1-2 natural spoken sentences per scene.
- Maximum 18 words per scene.
- Total narration should naturally fit 25-35 seconds.
- Write for spoken American English, not article prose.
- Avoid repetitive sentence openings and generic filler.

VISUAL RULES ARE CRITICAL:
- Every scene must have a DIFFERENT visual subject, composition and camera angle.
- Never reuse the same person, face, generic office, generic smartphone, generic
  laptop, generic city skyline or stock-photo pose unless the story genuinely requires it.
- Use documentary/editorial photography rather than polished corporate stock imagery.
- Show the actual physical subject, environment, evidence, location or consequence
  being discussed whenever possible.
- Do not place text, captions, logos, watermarks, UI screenshots or invented signage
  inside generated images.
- Use realistic anatomy, physically plausible lighting and natural imperfections.
- Prefer specific visual nouns over abstract concepts.
- Scene 1 should be visually arresting, Scene 2 action-oriented, Scene 3 wide/contextual,
  Scene 4 detail/macro, Scene 5 human/contextual or a memorable final wide shot.
- Do not repeat lighting, lens feel or framing across consecutive scenes.
...
{{
  ...
  "caption": "Facebook caption with useful context, one natural question and 2-3 relevant hashtags",
  "topic_key": "unique-three-word-slug"
}}
""", label="video_script")
    ...
    try:
        ...
        cleaned = []
        for index, scene in enumerate(slides[:5]):
            narration = " ".join(str(scene.get("narration", "")).split()).strip()
            image_prompt = " ".join(str(scene.get("image_prompt", "")).split()).strip()
            if not narration or not image_prompt:
                return None
            word_count = len(narration.split())
            if word_count > 18:
                log(f"⚠️ video_script scene {index + 1} rejected: {word_count} words exceeds 18-word limit")
                return None
            cleaned.append({
                "narration": narration,
                "image_prompt": image_prompt,
        ...
        caption = str(result.get("caption", "")).strip() or "🎬 Daily US update"
        topic_key = str(result.get("topic_key", "")).strip()
        if not topic_key:
            topic_key = "-".join(cleaned[0]["narration"].lower().split()[:3])

        result["slides"] = cleaned
        result["caption"] = caption[:2200]
        result["topic_key"] = topic_key[:120]
        return result
    except Exception as e:
...
def _generate_voiceover(full_narration_text, out_path):
    """Generate natural US narration with Edge-TTS."""
    ...
    async def _run():
        communicate = edge_tts.Communicate(
            full_narration_text,
            VIDEO_VOICE,
            rate=VIDEO_VOICE_RATE,
            pitch=VIDEO_VOICE_PITCH,
        )
...
def _escape_ffmpeg_text(text):
    """Escape text for FFmpeg drawtext/filtergraph arguments."""
    return (
        str(text)
        .replace("\\", "\\\\")
        .replace("'", "’")
...
def _wrap_video_caption(text, max_chars=40):
...
def _scene_durations(slides, total_duration, transition_seconds=VIDEO_TRANSITION_SECONDS):
    """Allocate scene lengths so transition overlap does not shorten the voice track."""
    if not slides:
        return []
    safe_total = max(VIDEO_MIN_DURATION, min(float(total_duration), VIDEO_MAX_DURATION))
    transition_total = max(0.0, float(transition_seconds)) * max(0, len(slides) - 1)
    target_scene_sum = safe_total + transition_total
    weights = []
    for slide in slides:
        words = str(slide.get("narration", "")).split()
        punctuation_bonus = sum(1 for word in words if word.endswith((",", ";", ":", ".", "?", "!")))
        weights.append(max(1.0, len(words) + punctuation_bonus * 0.35))
    total_weight = sum(weights)
    minimum = max(2.55, float(transition_seconds) * 2.2)
    if target_scene_sum < len(slides) * minimum:
        minimum = max(1.55, target_scene_sum / len(slides))
    durations = [max(minimum, target_scene_sum * weight / total_weight) for weight in weights]
    delta = sum(durations) - target_scene_sum
    for _ in range(200):
        if abs(delta) <= 0.01:
            break
        if delta > 0:
            candidates = [i for i, duration in enumerate(durations) if duration > minimum + 0.04]
            if not candidates:
                break
            index = max(candidates, key=lambda i: durations[i])
            step = min(delta, durations[index] - minimum)
            durations[index] -= step
            delta -= step
        else:
            index = max(range(len(durations)), key=lambda i: weights[i])
            step = min(-delta, 0.35)
            durations[index] += step
            delta += step
    return durations
...
def _ass_karaoke_duration(seconds):
    return max(1, int(round(float(seconds) * 100)))


def _word_durations(words, total_duration):
    """Allocate timing by characters and punctuation for smoother karaoke sync."""
    if not words:
        return []
    weights = []
    for word in words:
        clean = ''.join(ch for ch in word if ch.isalnum())
        weight = max(1.0, len(clean) * 0.82)
        if word.endswith((',', ';', ':')):
            weight += 0.35
        if word.endswith(('.', '?', '!')):
            weight += 0.75
    ...
    total = sum(weights)
    durations = [total_duration * weight / total for weight in weights]
    return [max(0.035, value) for value in durations]


def _build_ass_subtitles(slides, durations, ass_path, transition_seconds=VIDEO_TRANSITION_SECONDS):
    """Create professional two-line karaoke captions with scene labels."""
    labels = ["HOOK", "WHAT HAPPENED", "WHY IT MATTERS", "THE DETAIL", "TAKEAWAY"]
    header = """[Script Info]\nScriptType: v4.00+\nPlayResX: 1080\nPlayResY: 1920\nScaledBorderAndShadow: yes\nWrapStyle: 2\nYCbCr Matrix: TV.709\n\n[V4+ Styles]\nFormat: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding\nStyle: Caption,DejaVu Sans,54,&H0000D9FF,&H00FFFFFF,&H00101010,&H99000000,1,0,0,0,100,100,0,0,1,3,1,2,70,70,365,1\nStyle: Label,DejaVu Sans,27,&H00FFFFFF,&H00FFFFFF,&H00101010,&H88000000,1,0,0,0,100,100,0,0,1,2,1,8,70,70,92,1\nStyle: Top,DejaVu Sans,25,&H00FFFFFF,&H00FFFFFF,&H00101010,&H00000000,1,0,0,0,100,100,0,0,1,1,0,8,70,70,54,1\n\n[Events]\nFormat: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text\n"""
    events = []
    cursor = 0.0
    ...
    for index, (slide, duration) in enumerate(zip(slides, durations)):
        ...
        if index < len(durations) - 1:
            next_transition = min(
                ...
                max(0.08, duration / 3.0),
                max(0.08, durations[index + 1] / 3.0),
            )
            caption_end = max(start + 0.10, end - next_transition)
        else:
            caption_end = end
        words = str(slide.get("narration", "")).split()
        caption_duration = max(0.10, caption_end - start)
        word_durations = _word_durations(words, caption_duration)
        split_at = (len(words) + 1) // 2 if len(words) >= 8 else len(words)
        lines = [words[:split_at], words[split_at:]] if split_at < len(words) else [words]
        ...
        for line_index, line_words in enumerate(lines):
            ...
            for word in line_words:
                wd = word_durations[absolute_index]
                line_parts.append(
                    f"{{\\k{_ass_karaoke_duration(wd)}}}{_escape_ass_text(word)}"
                )
                if absolute_index < len(words) - 1:
                    line_parts.append(" ")
                absolute_index += 1
            parts.append("".join(line_parts))

        events.append(
            f"Dialogue: 1,{_ass_time(start)},{_ass_time(caption_end)},Caption,,0,0,365,,{r'\\N'.join(parts)}"
        )

        label = labels[index] if index < len(labels) else f"SCENE {index + 1}"
        label_text = _escape_ass_text(label)
        events.append(
            f"Dialogue: 0,{_ass_time(start)},{_ass_time(caption_end)},Label,,0,0,92,,{{\\fad(180,180)}}{label_text}"
        )
        events.append(
            f"Dialogue: 0,{_ass_time(start)},{_ass_time(caption_end)},Top,,0,0,54,,{{\\fad(220,220)}}AMERICAN STORY DESK"
        )
        if index < len(durations) - 1:
            cursor = end - min(
                float(transition_seconds),
    ...
    try:
        ...
        log(f"⚠️ _build_ass_subtitles failed: {e}")
        return False
...
def _video_render_profile():
    """Select a safe FFmpeg profile without depending on optional filters."""
    return {
        "crf": str(VIDEO_CRF),
...
def build_video_reel(slides, out_path):
    """Render a professional vertical Reel in one final H.264 encode."""
    if not FFMPEG_AVAILABLE:
        log("⚠️ build_video_reel skipped: ffmpeg is not installed.")
        return False
    if not slides:
        log("⚠️ build_video_reel skipped: no slides.")
        return False
    ...
    try:
        ...
        for index, slide in enumerate(slides):
            ...
            if not image_bytes:
                raise ValueError(f"slide {index + 1} has no image bytes")
            path = os.path.join(session_dir, f"scene_{index + 1}.jpg")
            with open(path, "wb") as handle:
                handle.write(image_bytes)
            image_paths.append(path)

        full_narration = ". ".join(
            str(slide.get("narration", "")).strip().rstrip(".!?") + "."
            for slide in slides
        ).strip()
        voice_path = os.path.join(session_dir, "voice.mp3")
        has_voice = _generate_voiceover(full_narration, voice_path)
        if VIDEO_REQUIRE_VOICE and not has_voice:
            log("⚠️ Professional Reel render aborted: voiceover generation failed.")
            return False

        if has_voice:
            voice_duration = _get_media_duration(voice_path)
            if voice_duration > VIDEO_MAX_DURATION + 0.75:
                log(
                    f"⚠️ Professional Reel render aborted: voiceover is too long "
                    f"({voice_duration:.2f}s > {VIDEO_MAX_DURATION:.2f}s)."
                )
                return False
            total_duration = max(VIDEO_MIN_DURATION, min(voice_duration, VIDEO_MAX_DURATION))
        else:
            total_duration = max(VIDEO_MIN_DURATION, min(len(slides) * 4.0, VIDEO_MAX_DURATION))

        durations = _scene_durations(slides, total_duration, VIDEO_TRANSITION_SECONDS)
        if not durations:
            raise ValueError("no scene durations")

        ass_path = os.path.join(session_dir, "captions.ass")
        ass_ok = _build_ass_subtitles(slides, durations, ass_path, VIDEO_TRANSITION_SECONDS)
        use_ass = bool(ass_ok and _probe_ffmpeg_libass())

        filter_parts = []
        video_labels = []
        transitions = ["fade", "smoothleft", "smoothright", "wipeleft", "wiperight"]
        ...
        for index, (image_path, duration) in enumerate(zip(image_paths, durations)):
            ...
            mode = index % 5
            if mode == 0:
                zoom = "min(max(zoom+0.0010,1.0),1.20)"
                x_expr = "iw/2-(iw/zoom/2)"
                y_expr = "ih/2-(ih/zoom/2)"
            elif mode == 1:
                zoom = "max(min(zoom-0.0009,1.20),1.0)"
                x_expr = "iw/2-(iw/zoom/2)"
                y_expr = "ih/2-(ih/zoom/2)"
            elif mode == 2:
                zoom = "min(max(zoom+0.00075,1.0),1.16)"
                x_expr = "iw/2-(iw/zoom/2)-min(95,iw/zoom*0.075)"
                y_expr = "ih/2-(ih/zoom/2)"
            elif mode == 3:
                zoom = "min(max(zoom+0.00065,1.0),1.14)"
                x_expr = "iw/2-(iw/zoom/2)+min(90,iw/zoom*0.07)"
                y_expr = "ih/2-(ih/zoom/2)-min(70,ih/zoom*0.05)"
            else:
                zoom = "max(min(zoom-0.0007,1.16),1.0)"
                x_expr = "iw/2-(iw/zoom/2)"
                y_expr = "ih/2-(ih/zoom/2)+min(70,ih/zoom*0.05)"

            vf = [
                f"[{index}:v]scale=1180:2100:force_original_aspect_ratio=increase:flags=lanczos",
                "crop=1080:1920",
            ...
            label = f"v{index}"
            filter_parts.append(",".join(vf) + f"[{label}]")
        ...
        for index in range(1, len(video_labels)):
            transition_duration = min(
                ...
                max(0.08, durations[index - 1] / 3.0),
                max(0.08, durations[index] / 3.0),
            )
            offset = max(0.01, elapsed - transition_duration)
            out_label = f"vx{index}"
            transition = transitions[(index - 1) % len(transitions)]
            filter_parts.append(
                f"[{current}][{video_labels[index]}]xfade="
                f"transition={transition}:duration={transition_duration:.3f}:offset={offset:.3f}[{out_label}]"
            )
            current = out_label
            elapsed += durations[index] - transition_duration

        render_duration = max(0.1, elapsed)
        filter_parts.append(
            f"[{current}]drawbox=x=54:y=52:w=972:h=8:color=black@0.22:t=fill[frame]"
        )
        filter_parts.append(
            f"[frame]drawbox=x=54:y=52:w='972*min(1,max(0,t/{render_duration:.3f}))':h=8:color=white@0.90:t=fill[progress]"
        )
        current = "progress"

        # Editorial safe-zone shading keeps captions legible over bright footage.
        filter_parts.append(
            f"[{current}]drawbox=x=0:y=1510:w=1080:h=410:color=black@0.10:t=fill[shade]"
        )
        current = "shade"

        if use_ass:
            subtitle_path = _ffmpeg_subtitle_path(ass_path)
            filter_parts.append(
                f"[{current}]subtitles=filename='{subtitle_path}':"
                "force_style='FontName=DejaVu Sans,FontSize=54,"
                "PrimaryColour=&H0000D9FF,SecondaryColour=&H00FFFFFF,"
                "OutlineColour=&H00101010,BackColour=&H99000000,"
                "BorderStyle=1,Outline=3,Shadow=1,Alignment=2,"
                "MarginL=70,MarginR=70,MarginV=365'"
                "[captioned]"
            )
            current = "captioned"
        elif _VIDEO_FONT:
            # Fallback for FFmpeg builds without libass.
            cursor = 0.0
            for index, (slide, duration) in enumerate(zip(slides, durations)):
                caption = _escape_ffmpeg_text(_wrap_video_caption(slide.get("narration", "")))
                end = cursor + duration
                filter_parts.append(
                    f"[{current}]drawtext=fontfile={_VIDEO_FONT}:"
                    f"text='{caption}':fontsize={VIDEO_CAPTION_FONT_SIZE}:"
                    "fontcolor=white:box=1:boxcolor=black@0.56:boxborderw=18:"
                    "shadowcolor=black@0.85:shadowx=2:shadowy=2:"
                    f"x=(w-text_w)/2:y=h-{VIDEO_CAPTION_MARGIN_V}:"
                    f"enable='between(t,{cursor:.3f},{end:.3f})'[cap{index}]"
                )
                current = f"cap{index}"
                cursor = end

        # Start/end fades are subtle enough not to look like a template.
        filter_parts.append(
            f"[{current}]fade=t=in:st=0:d=0.16,fade=t=out:st={max(0.16, render_duration - 0.16):.3f}:d=0.16,format=yuv420p[vout]"
        )

        command = ["ffmpeg", "-hide_banner", "-loglevel", "warning", "-y"]
        for path in image_paths:
            command += ["-loop", "1", "-i", path]
        if has_voice:
            command += ["-i", voice_path]
        music_enabled = bool(VIDEO_MUSIC_FILE and os.path.isfile(VIDEO_MUSIC_FILE))
        if music_enabled:
            command += ["-stream_loop", "-1", "-i", VIDEO_MUSIC_FILE]

        filter_complex = ";".join(filter_parts)
        audio_filter = ""
        if has_voice:
            voice_index = len(image_paths)
            if music_enabled:
                music_index = voice_index + 1
                audio_filter = (
                    f";[{voice_index}:a]highpass=f=70,lowpass=f=12500,"
                    "acompressor=threshold=-19dB:ratio=2.8:attack=8:release=130,"
                    "loudnorm=I=-16:TP=-1.5:LRA=10[voice];"
                    f"[{music_index}:a]atrim=duration={render_duration:.3f},"
                    "afade=t=in:st=0:d=1.2,"
                    f"afade=t=out:st={max(0.2, render_duration - 1.5):.3f}:d=1.3,"
                    f"volume={VIDEO_MUSIC_VOLUME:.4f},highpass=f=80,lowpass=f=9000[music];"
                    "[voice][music]amix=inputs=2:duration=first:dropout_transition=2,"
                    "loudnorm=I=-16:TP=-1.5:LRA=10[aout]"
                )
            else:
                audio_filter = (
                    f";[{voice_index}:a]highpass=f=70,lowpass=f=12500,"
        ...
        if has_voice:
            command += ["-map", "[aout]", "-c:a", "aac", "-b:a", "160k", "-ar", "48000"]
        else:
            command += ["-an"]

        profile = _video_render_profile()
        command += [
        ...
        if result.returncode != 0 or not os.path.exists(out_path) or os.path.getsize(out_path) < 20000:
            stderr = result.stderr.decode(errors="ignore")
            log(f"⚠️ FFmpeg professional Reel render failed: {stderr[-1800:]}")
            return False

        # Validate the final artifact before anything is uploaded.
        ...
        if probe.returncode != 0:
            log("⚠️ Final Reel ffprobe validation failed.")
            return False
        try:
            ...
            if int(video_stream.get("width", 0)) != VIDEO_WIDTH or int(video_stream.get("height", 0)) != VIDEO_HEIGHT:
                log(f"⚠️ Invalid Reel dimensions: {video_stream.get('width')}x{video_stream.get('height')}")
                return False
            if video_stream.get("codec_name") != "h264":
                log(f"⚠️ Invalid Reel codec: {video_stream.get('codec_name')}")
                return False
            if has_voice and not audio_stream:
                log("⚠️ Voice was generated but final Reel contains no audio stream.")
                return False
            if has_voice and audio_stream:
                final_duration = float(video_stream.get("duration") or 0.0)
                audio_final_duration = float(audio_stream.get("duration") or 0.0)
                if final_duration < max(1.0, total_duration - 0.45):
                    log(
                        f"⚠️ Final Reel is shorter than the narration target: "
                        f"video={final_duration:.2f}s target={total_duration:.2f}s"
                    )
                    return False
                if abs(final_duration - audio_final_duration) > 0.45:
                    log(
                        f"⚠️ Final Reel A/V duration mismatch: "
                        f"video={final_duration:.2f}s audio={audio_final_duration:.2f}s"
                    )
                    return False
        except Exception as e:
            log(f"⚠️ Final Reel metadata validation failed: {e}")
            return False
...
def _cover_text_lines(text, max_words=8):
    ...
    return (
        " ".join(words[:split_at]),
        " ".join(words[split_at:]).rstrip(".,!?;:") + "…",
    )


def build_reel_cover(slides, cover_path):
    """Build a real, high-contrast 1080x1920 thumbnail from the hook image."""
    ...
    try:
        ...
        source_path = os.path.join(
            VIDEO_TEMP_DIR,
            f"cover_source_{int(time.time() * 1000)}_{random.randint(100,999)}.jpg",
        )
        with open(source_path, "wb") as handle:
            handle.write(slides[0]["image_bytes"])

        line1, line2 = _cover_text_lines(slides[0].get("narration", ""), 8)
        subtitle = _cover_text_lines(slides[1].get("narration", ""), 7)[0] if len(slides) > 1 else "The key facts in seconds"
        line1 = _escape_ffmpeg_text(line1[:60])
        line2 = _escape_ffmpeg_text(line2[:60])
        subtitle = _escape_ffmpeg_text(subtitle[:72])

        vf = [
            "scale=1080:1920:force_original_aspect_ratio=increase:flags=lanczos",
            "crop=1080:1920",
            "eq=brightness=-0.08:contrast=1.10:saturation=1.12",
            "unsharp=5:5:0.35:5:5:0.0",
            "vignette=PI/5",
            "drawbox=x=0:y=0:w=1080:h=420:color=black@0.18:t=fill",
            "drawbox=x=55:y=590:w=970:h=800:color=black@0.56:t=fill",
            "drawbox=x=82:y=742:w=8:h=450:color=white@0.95:t=fill",
            "drawbox=x=82:y=742:w=330:h=8:color=white@0.95:t=fill",
            "drawbox=x=82:y=1238:w=330:h=8:color=white@0.70:t=fill",
        ]

        if _VIDEO_FONT:
            vf.extend([
                f"drawtext=fontfile={_VIDEO_FONT}:text='AMERICAN STORY DESK':fontsize=30:fontcolor=white@0.94:x=82:y=82:borderw=1:bordercolor=black@0.7",
                f"drawtext=fontfile={_VIDEO_FONT}:text='LATEST • EXPLAINED':fontsize=28:fontcolor=white@0.78:x=82:y=126:borderw=1:bordercolor=black@0.55",
                f"drawtext=fontfile={_VIDEO_FONT}:text='{line1}':fontsize=68:fontcolor=white:x=126:y=790:borderw=4:bordercolor=black@0.88",
                f"drawtext=fontfile={_VIDEO_FONT}:text='{line2}':fontsize=68:fontcolor=white:x=126:y=900:borderw=4:bordercolor=black@0.88",
                f"drawtext=fontfile={_VIDEO_FONT}:text='{subtitle}':fontsize=37:fontcolor=white@0.94:x=126:y=1060:borderw=2:bordercolor=black@0.72",
                f"drawtext=fontfile={_VIDEO_FONT}:text='FACTS • CONTEXT • 30 SECONDS':fontsize=27:fontcolor=white@0.76:x=82:y=1285:borderw=1:bordercolor=black@0.55",
            ])
        ...
        result = subprocess.run(
            [
                ...
                "-vf", ",".join(vf),
                "-frames:v", "1",
                "-q:v", str(VIDEO_COVER_QUALITY),
                cover_path,
            ],
            capture_output=True,
            timeout=60,
        )
        if result.returncode != 0 or not os.path.exists(cover_path) or os.path.getsize(cover_path) < 5000:
            log(f"⚠️ Reel cover generation failed: {result.stderr.decode(errors='ignore')[-800:]}")
            return False
        ...
        return int(stream.get("width", 0)) == 1080 and int(stream.get("height", 0)) == 1920
    except Exception as e:
...
def post_video_reel(video_path, caption, cover_path=None):
    """Upload the finished Reel using the existing Meta three-step flow."""
    GRAPH_VERSION = "v20.0"

    if not FB_PAGE_ID or not FB_ACCESS_TOKEN:
        return {"error": {"message": "FB_PAGE_ID or FB_ACCESS_TOKEN is missing"}}
    if not os.path.exists(video_path) or os.path.getsize(video_path) < 20000:
        return {"error": {"message": "Video file is missing or invalid"}}
    ...
    try:
        ...
        try:
            start_resp = start_response.json()
        except Exception:
            start_resp = {"raw": start_response.text[:500]}
        if start_response.status_code >= 400 or "video_id" not in start_resp:
            return {"error": {"message": f"Reel start failed: {start_resp}"}}

        video_id = start_resp["video_id"]
        upload_url = start_resp.get("upload_url") or f"https://rupload.facebook.com/video-upload/{GRAPH_VERSION}/{video_id}"
        file_size = os.path.getsize(video_path)
        with open(video_path, "rb") as handle:
            video_bytes = handle.read()

        upload_response = None
        last_upload_error = "unknown upload error"
        for upload_attempt in range(1, 4):
            try:
                ...
                if upload_response.status_code in (200, 201):
                    break
                last_upload_error = upload_response.text[:500]
                log(f"⚠️ Reel upload attempt {upload_attempt}/3 failed: HTTP {upload_response.status_code} {last_upload_error}")
            except requests.RequestException as upload_error:
                last_upload_error = str(upload_error)
                log(f"⚠️ Reel upload attempt {upload_attempt}/3 exception: {upload_error}")
            if upload_attempt < 3:
                time.sleep(3 * upload_attempt)
        if upload_response is None or upload_response.status_code not in (200, 201):
            return {"error": {"message": f"Reel upload failed after 3 attempts: {last_upload_error}"}}
        ...
        try:
            finish_resp = finish_response.json()
        except Exception:
            finish_resp = {"raw": finish_response.text[:500]}
        ...
        if cover_path and os.path.exists(cover_path):
            try:
                ...
                if thumb_response.status_code not in (200, 201):
                    log(f"⚠️ Reel cover upload was rejected; Reel remains published: {thumb_response.text[:500]}")
                else:
                    try:
                    ...
                    if isinstance(thumb_json, dict) and thumb_json.get("error"):
                        log(f"⚠️ Reel cover upload returned an error; Reel remains published: {thumb_json}")
                    else:
        ...
        return {"error": {"message": f"Facebook Reel request failed: {e}"}}
    except Exception as e:
        return {"error": {"message": str(e)}}


def _video_enhanced_image_prompt(slide_data, scene_index, topic_key, rejected_reason=None):
    """Turn a model prompt into a deterministic cinematic production brief."""
    role = VIDEO_SCENE_ROLES[min(scene_index, len(VIDEO_SCENE_ROLES) - 1)]
    retry_note = ""
    if rejected_reason:
        retry_note = (
            " IMPORTANT: the previous candidate looked too similar to another Reel scene. "
            "Change the subject framing, lens perspective, foreground/background relationship, "
            "lighting direction and dominant visual shape. Do not merely recolor the same image."
        )
    return (
        f"{slide_data['image_prompt']}. "
        f"Production role: {role['name']}. Required shot: {role['shot']}. "
        f"Camera language: {role['camera']}. Suggested motion feeling: {role['motion']}. "
        "Premium documentary/editorial photography, photorealistic, physically plausible, "
        "natural skin and material texture, realistic depth, restrained cinematic color grade, "
        "authentic environment, no text, no letters, no captions, no logos, no watermarks, "
        "no split screen, no collage, no duplicate subject, no generic stock pose. "
        f"This is scene {scene_index + 1} of a five-scene story with topic key {topic_key}. "
        "The image must communicate the narration immediately and must look visually distinct "
        "from the other scenes in this same Reel and from recent generated Reels."
        f"{retry_note}"
    )


def _generate_video_scene_image(slide_data, scene_index, topic_key, memory, current_fingerprints):
    """Generate a scene image and reject perceptually repetitive candidates."""
    last_reason = None
    for attempt in range(VIDEO_MAX_IMAGE_RETRIES + 1):
        prompt = _video_enhanced_image_prompt(
            slide_data,
            scene_index,
            topic_key,
        ...
        if not image or not image.get("bytes"):
            last_reason = "image generation failed"
            continue

        fingerprint = _video_image_fingerprint(image["bytes"])
        if fingerprint and _video_image_is_too_similar(fingerprint, memory, current_fingerprints):
            last_reason = "perceptual duplicate detected"
            log(
                f"⚠️ Video scene {scene_index + 1}: rejected visually similar image "
                f"on candidate {attempt + 1}."
            )
            continue
...
# Video generation is deliberately isolated from the main scheduler. A Reel can
# involve network calls, image generation, Edge-TTS, FFmpeg encoding and a
# multi-step Facebook upload; none of those operations are allowed to block the
# legacy post scheduler.
VIDEO_JOB_QUEUE = Queue(maxsize=1)
VIDEO_PENDING_JOBS = set()
VIDEO_RETRY_AFTER = {}
VIDEO_RETRY_COUNT = {}
VIDEO_JOB_LOCK = Lock()
VIDEO_WORKER_THREAD = None
VIDEO_MAX_RETRIES_PER_SLOT = 2
VIDEO_CATCHUP_MINUTES = 10
VIDEO_RETRY_DELAY_SECONDS = 180


def _video_job_key(slot_date, slot_index):
    return (str(slot_date), int(slot_index))
...
def _video_worker():
    while True:
        try:
            ...
            slot_date, slot_index = job
            key = _video_job_key(slot_date, slot_index)
            try:
                log(
                    f"🎬 Video worker started slot {slot_index + 1} "
                    f"for {slot_date}."
                )
                success = bool(generate_and_post_video())
                today = now().strftime("%Y-%m-%d")

                with VIDEO_JOB_LOCK:
                    VIDEO_PENDING_JOBS.discard(key)
                    if success and slot_date == today:
                        posted_video_slots.add(int(slot_index))
                        VIDEO_RETRY_AFTER.pop(key, None)
                        VIDEO_RETRY_COUNT.pop(key, None)
                    elif not success and slot_date == today:
                        attempts = VIDEO_RETRY_COUNT.get(key, 0) + 1
                        VIDEO_RETRY_COUNT[key] = attempts
                        if attempts <= VIDEO_MAX_RETRIES_PER_SLOT:
                            VIDEO_RETRY_AFTER[key] = time.time() + VIDEO_RETRY_DELAY_SECONDS
                            log(
                                f"⚠️ Video slot {slot_index + 1} failed. "
                                f"Retry {attempts}/{VIDEO_MAX_RETRIES_PER_SLOT} "
                                f"scheduled in {VIDEO_RETRY_DELAY_SECONDS // 60} minutes."
                            )
                        else:
                            VIDEO_RETRY_AFTER.pop(key, None)
                            log(
                                f"❌ Video slot {slot_index + 1} exhausted its "
                                f"{VIDEO_MAX_RETRIES_PER_SLOT} retry attempts for today."
                            )

                # The scheduler persists state every normal loop. This additional
                # save is intentionally outside the scheduler's posting path and
                # only records the completed video slot after a successful publish.
                if success and slot_date == today:
                    try:
                        save_state()
                    except Exception as state_error:
                        log(f"⚠️ Video worker state save failed: {state_error}")

                if success:
                    log(
                        f"✅ Video worker completed slot {slot_index + 1} "
                        f"for {slot_date}."
                    )
                else:
                    log(
                        f"⚠️ Video worker did not publish slot {slot_index + 1} "
                        f"for {slot_date}."
                    )
            except Exception as worker_error:
                log(f"🔥 Video worker job crashed safely: {worker_error}")
                with VIDEO_JOB_LOCK:
                    VIDEO_PENDING_JOBS.discard(key)
                    today = now().strftime("%Y-%m-%d")
                    if slot_date == today:
            ...
            log(f"🔥 Video worker loop recovered from error: {loop_error}")
            time.sleep(2)
...
def _start_video_worker():
    ...
    with VIDEO_JOB_LOCK:
        if VIDEO_WORKER_THREAD is not None and VIDEO_WORKER_THREAD.is_alive():
            return
        VIDEO_WORKER_THREAD = Thread(
            target=_video_worker,
            name="fb-video-worker",
            daemon=True,
        )
        VIDEO_WORKER_THREAD.start()
    log("🎬 Video background worker started; main scheduler remains non-blocking.")


def _queue_video_job(slot_index, slot_date):
    key = _video_job_key(slot_date, slot_index)
    with VIDEO_JOB_LOCK:
        if key in VIDEO_PENDING_JOBS:
            return False
        if key in VIDEO_RETRY_AFTER and time.time() < VIDEO_RETRY_AFTER[key]:
            return False
        if int(slot_index) in posted_video_slots:
            return False
        VIDEO_PENDING_JOBS.add(key)

    try:
        VIDEO_JOB_QUEUE.put_nowait((str(slot_date), int(slot_index)))
        log(
            f"📥 Video slot {int(slot_index) + 1} queued for background generation."
        )
        return True
    except Full:
        with VIDEO_JOB_LOCK:
            VIDEO_PENDING_JOBS.discard(key)
        log("⚠️ Video worker is busy; current slot remains eligible for the next scheduler pass.")
        return False
    except Exception as queue_error:
        with VIDEO_JOB_LOCK:
            VIDEO_PENDING_JOBS.discard(key)
        log(f"⚠️ Failed to queue video job: {queue_error}")
        return False


def generate_and_post_video(topic_hint=None):
    """Generate, render, validate and publish one professional Reel transactionally."""
    if not FFMPEG_AVAILABLE or not EDGE_TTS_AVAILABLE:
        missing = []
        if not FFMPEG_AVAILABLE:
            missing.append("ffmpeg")
        if not EDGE_TTS_AVAILABLE:
            missing.append("edge-tts")
        log("⚠️ Video post skipped — missing: " + ", ".join(missing))
        return False
    ...
    topic_key = str(script.get("topic_key", "")).strip() or str(random.randint(1, 999999))
    memory = _video_image_memory_load()
    current_fingerprints = []
    pending_memory_entries = []
    slides = []

    for index, slide_data in enumerate(script["slides"][:5]):
        image, fingerprint = _generate_video_scene_image(
            slide_data,
            index,
            topic_key,
            memory,
        ...
        if not image or not image.get("bytes"):
            log(
                f"⚠️ generate_and_post_video: scene {index + 1}/5 image generation failed "
                "after duplicate-aware retries; video skipped."
            )
            return False
    ...
    session_suffix = random.randint(1000, 9999)
    out_path = os.path.join(VIDEO_TEMP_DIR, f"reel_{stamp}_{session_suffix}.mp4")
    cover_path = os.path.join(VIDEO_TEMP_DIR, f"reel_cover_{stamp}_{session_suffix}.jpg")

    try:
        cover_ok = build_reel_cover(slides, cover_path)
        if not build_video_reel(slides, out_path):
            log("❌ generate_and_post_video: professional video render failed.")
            return False
        ...
        if "id" in result:
            # Commit topic/image memory only after the Reel was actually published.
            seen_video_topics.add(topic_key)
            if pending_memory_entries:
                committed_memory = _video_image_memory_load()
                committed_memory.extend(pending_memory_entries)
                _video_image_memory_save(committed_memory)

            log(f"✅ Professional Video Reel posted: {result['id']}")
        ...
        for cleanup_path in (out_path, cover_path):
            try:
                ...
                log(f"⚠️ Video cleanup failed for {cleanup_path}: {e}")
    ...
    while True:
        try:
            if reset_time() and last_reset_date != now().strftime("%Y-%m-%d"):
                ...
                with VIDEO_JOB_LOCK:
                    VIDEO_RETRY_AFTER.clear()
                    VIDEO_RETRY_COUNT.clear()
                daily_posted_posts = []
            ...
            # The professional Reel pipeline runs in its own worker so image
            # generation, TTS, FFmpeg and Facebook upload can never block the
            # legacy scheduler or make other post categories miss their slots.
            # A small catch-up window protects a video slot from being lost if
            # the scheduler was briefly busy or the process restarted near the
            # scheduled minute. Retries are limited and isolated to video only.
            current_time = now()
            ...
            for i, (h, m) in enumerate(_jittered_video_slots):
                ...
                minutes_late = current_minute - scheduled_minute
                key = _video_job_key(current_date, i)
                retry_due = False
                with VIDEO_JOB_LOCK:
                    retry_due = (
                        key in VIDEO_RETRY_AFTER
                        and time.time() >= VIDEO_RETRY_AFTER[key]
                    )
                    pending = key in VIDEO_PENDING_JOBS

                scheduled_due = 0 <= minutes_late <= VIDEO_CATCHUP_MINUTES
                if (scheduled_due or retry_due) and not pending:
                    _queue_video_job(i, current_date)
