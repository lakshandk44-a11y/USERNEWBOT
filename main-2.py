...
# Pillow (PIL) = used ONLY to measure the real rendered pixel width of
# burned-in karaoke captions against the actual font file, so a caption line
# never runs off the edge of the 1080px-wide frame. Fully optional: if it
# isn't installed, caption line-breaking falls back to a conservative
# character-count estimate instead (still safe, just slightly less precise).
...
# Used by cartoon_generate()'s fallback path (when Gemini/JSON parsing
# fails) to keep the same "agree or disagree?" opinionated tone its normal
# AI-written captions ask for — a genuine reaction prompt, not engagement-
# farming ("tag 3 friends", "share if you agree").
ENGAGEMENT_CTAS = [
    "Agree or disagree? 👇",
...
# Keep the established natural US voice as the default; rate can be tuned without
# changing the API flow.  A local/approved music file is optional and disabled by default.
...
VIDEO_MUSIC_VOLUME = float(os.getenv("VIDEO_MUSIC_VOLUME", "0.055"))
# Optional small corner watermark/handle (e.g. "@YourPage") — a common
# finishing touch on professional short-form edits. Off by default: if this
# is left unset, nothing about the rendered video changes at all.
VIDEO_WATERMARK_TEXT = os.getenv("VIDEO_WATERMARK_TEXT", "").strip()

# Common font locations across Debian/Ubuntu/macOS/Windows — first one found
# is used for burned-in captions AND the reel cover/thumbnail text. If truly
# none exist anywhere on the box, text is skipped (voiceover + images still
# work fine without it) — but this is now a last resort, not the common case.
_FONT_CANDIDATES = [
    ...
    ("/usr/share/fonts/truetype/liberation2/LiberationSans-Bold.ttf", "Liberation Sans"),
    ...
    ("/usr/share/fonts/truetype/msttcorefonts/Arial_Bold.ttf", "Arial"),
...
def _discover_video_font():
    """Finds a usable bold sans-serif font file + its real font FAMILY name
    (needed separately by libass captions) + its directory (needed for
    libass's fontsdir). Falls back to asking the OS via `fc-list` for ANY
    installed TTF/OTF font if none of the hardcoded distro paths match, so a
    server with fonts installed under an unexpected path still gets working
    captions/thumbnail text instead of silently rendering nothing."""
    for path, family in _FONT_CANDIDATES:
        if os.path.exists(path):
            return path, family, os.path.dirname(path)
    try:
        ...
        if found.returncode == 0:
            all_lines = [ln for ln in found.stdout.splitlines() if ln.strip()]
            bold_lines = [ln for ln in all_lines if "bold" in ln.lower()]
            for ln in (bold_lines or all_lines):
                try:
                    file_part, family_part = ln.split(":", 1)
                    file_part = file_part.strip()
                    family = family_part.split(",")[0].strip() or "Sans"
                    if file_part.lower().endswith((".ttf", ".otf")) and os.path.exists(file_part):
                        return file_part, family, os.path.dirname(file_part)
                except Exception:
                    continue
    except Exception:
        pass
...
_VIDEO_FONT, _VIDEO_FONT_FAMILY, _VIDEO_FONT_DIR = _discover_video_font()
if not _VIDEO_FONT:
    log("⚠️ No usable font found for video captions/thumbnail text. "
...
# Backward-compatible alias for any code that still refers to VIDEO_TOPICS.
VIDEO_TOPICS = VIDEO_CATEGORIES

def _pick_video_topic(video_index=None):
    """Choose a topic without adding any extra API call.
    Prefer an existing US-relevant news item; otherwise use a local category.
    For two daily videos, prefer different local categories."""
    try:
        stories = get_news()
        candidates = [x for x in stories if x.get("_us_relevance", 0) > 0]
        if candidates:
            # Existing news request only; no new network/API request.
            return random.choice(candidates)["title"]
    except Exception:
        pass

    # Local-only fallback: no API/VPS cost.
...
def generate_video_script(topic_hint=None):
    """Generate one clear, narrated Reel script without adding extra API calls."""
    topic = topic_hint or _pick_video_topic()
    text = call_gemini(f"""
You are a professional US short-form news/explainer video writer.

Create ONE original, easy-to-understand 25-35 second faceless Reel about:
{topic}

The viewer should understand the subject even with no caption context.
Do not invent facts, names, numbers, quotes, events or causes. If the topic is
uncertain or breaking, use cautious wording such as "reports indicate" rather
than pretending something is confirmed.

Structure: exactly 5 scenes:
1) HOOK — clear, specific, curiosity without misleading clickbait.
2) WHAT HAPPENED — the key fact in plain English.
3) WHY IT MATTERS — explain the practical meaning.
4) IMPORTANT DETAIL — one useful detail, number or context if supported.
5) TAKEAWAY — one concise closing sentence.

Rules:
- Each narration is 1-2 natural spoken sentences, max 18 words per scene.
- Use punctuation that sounds natural when spoken. Avoid robotic list-reading.
- Total narration should sound natural in about 25-35 seconds.
- Each image_prompt must describe a DIFFERENT visual scene that directly matches
  what is being said. Use photorealistic documentary/editorial photography,
  realistic people/places/objects, believable lighting, natural imperfections,
  coherent time/place, and no text, letters, logos or watermarks.
- Vary the visual composition across scenes: close-up, medium/action, wide
  establishing shot, detail shot, and contextual scene. Do not repeat the same
  camera angle or generic "person looking at camera" composition.
- Avoid impossible anatomy, surreal objects, fake screens, and generic stock-photo poses.
- caption: concise Facebook caption that restates the subject, gives useful
  context, asks ONE natural question, and uses 2-3 relevant hashtags.
  No like/share/follow bait, no engagement farming, no exaggerated claims.
- topic_key: unique 3-word slug for deduplication.

Return ONLY JSON:
{{
  "slides": [
    {{"narration": "hook", "image_prompt": "..."}},
    {{"narration": "what happened", "image_prompt": "..."}},
    {{"narration": "why it matters", "image_prompt": "..."}},
    {{"narration": "important detail", "image_prompt": "..."}},
    {{"narration": "takeaway", "image_prompt": "..."}}
  ],
  "caption": "Facebook caption with 2-3 relevant hashtags",
  "topic_key": "slug-here"
}}
""", label="video_script")
    ...
    try:
        ...
        if len(slides) < 5:
            return None
        # Normalize missing fields instead of letting malformed AI output break the video.
        cleaned = []
        for s in slides[:5]:
...
def _generate_voiceover(full_narration_text, out_path):
    """Uses edge-tts for a natural US narration; no additional AI API call."""
...
def _generate_voiceover_with_timings(full_narration_text, out_path):
    """
    Same voiceover as _generate_voiceover(), but uses edge-tts's streaming
    API to ALSO capture the exact start/end timestamp of every spoken word
    (edge-tts emits native "WordBoundary" events during synthesis — this is
    not estimated/guessed, it's the real timing straight from the TTS engine).

    Returns (audio_ok: bool, word_timings: list[{"word","start","end"}]).
    word_timings is [] if streaming/parsing fails for any reason — in that
    case the caller falls back to the old static caption style, and the
    audio itself is regenerated with the simple non-streaming path so a
    word-timing hiccup never costs the whole video.
    """
    if not EDGE_TTS_AVAILABLE:
        return False, []
    ...
    try:
        asyncio.run(_run())
        audio_ok = os.path.exists(out_path) and os.path.getsize(out_path) > 1000
        if not audio_ok:
            return False, []
        # Sanity check: timings must be sorted and non-overlapping-ish; if the
        # engine returned anything malformed, don't risk broken captions.
        for i in range(1, len(word_timings)):
            if word_timings[i]["start"] < word_timings[i - 1]["start"]:
                log("⚠️ _generate_voiceover_with_timings: out-of-order word timings, dropping timings.")
                return True, []
        return True, word_timings
    except Exception as e:
        log(f"⚠️ _generate_voiceover_with_timings streaming failed, falling back to plain TTS: {e}")
        try:
            ok = _generate_voiceover(full_narration_text, out_path)
            return ok, []
        except Exception as e2:
            log(f"⚠️ _generate_voiceover fallback also failed: {e2}")
            return False, []


def _ass_escape(text):
    """Escapes text for safe embedding inside an .ass Dialogue line."""
    return (str(text)
            .replace("\\", "\\\\")
            .replace("{", "\\{")
            .replace("}", "\\}")
...
def _escape_ffmpeg_filter_path(path):
    ...
    option value (e.g. ass=filename='...'), where ':' , '\\' and "'" are
    special characters to the filtergraph parser."""
    return (str(path)
            .replace("\\", "\\\\")
            .replace(":", "\\:")
            .replace("'", "\\'"))
...
KARAOKE_FONT_SIZE = 66
KARAOKE_MARGIN_L = 60
KARAOKE_MARGIN_R = 60
KARAOKE_PLAY_RES_X = 1080
_KARAOKE_FONT_CACHE = {}


def _measure_caption_text_width(text, fontsize):
    """Best-effort REAL rendered pixel width of `text` at `fontsize`, using
    the actual font file burned-in captions use. Returns None if Pillow
    isn't installed or the font can't be loaded, so the caller falls back to
    a conservative character-count estimate instead of guessing wrong."""
    if not PIL_AVAILABLE or not _VIDEO_FONT:
        return None
    key = (_VIDEO_FONT, fontsize)
    font = _KARAOKE_FONT_CACHE.get(key)
    if font is None:
        try:
            font = _PILImageFont.truetype(_VIDEO_FONT, fontsize)
        except Exception:
            font = False  # cache the failure too, so we don't retry every call
        _KARAOKE_FONT_CACHE[key] = font
    if not font:
        return None
    try:
...
def _build_karaoke_ass(word_timings, ass_path, words_per_line=5, max_line_chars=20,
                        scene_word_counts=None):
    """
    Builds a TikTok/CapCut-style karaoke subtitle (.ass) file: short caption
    lines of a few words, with the word currently being spoken rendered
    larger/bold in an accent color while the rest of the line stays white.
    Timing comes directly from real edge-tts word boundaries, so the
    highlight is genuinely synced to the voice rather than guessed.

    Line breaks respect three constraints, since WrapStyle=2 below means
    libass will NOT wrap a too-long line for us — it will simply run off
    both edges of the frame:
      1. real rendered pixel width against the actual caption font (via
         Pillow, when available) vs. a safety-margined budget
      2. a conservative character-count estimate as a fallback when Pillow
         isn't installed, so overflow is prevented either way
      3. a hard break at every scene/sentence boundary (when the caller
         supplies scene_word_counts), so a line never glues the tail of one
         narration sentence to the head of the next
    """
    if not word_timings:
        return False

    # Usable width is the frame minus both side margins; additionally
    # undershoot that by ~18% because the ASS style below also sets Bold=1
    # on top of an already-bold font file (synthetic double-bold), which
    # renders visibly wider than a plain single-bold measurement would
    # suggest — this margin is what keeps real output safely on-screen.
    safe_pixel_width = (KARAOKE_PLAY_RES_X - KARAOKE_MARGIN_L - KARAOKE_MARGIN_R) * 0.82

    # Precompute the word-index that starts each new narration scene, so a
    # caption line is never built out of the tail of one sentence glued to
    # the head of the next one.
    hard_break_before = set()
    if scene_word_counts:
        idx = 0
        for count in scene_word_counts[:-1]:
            idx += count
            if 0 < idx < len(word_timings):
                hard_break_before.add(idx)

    # Group words into short, phone-readable lines that are guaranteed to
    # fit on-screen and never straddle a scene boundary.
    lines, current, current_len = [], [], 0
    for i, w in enumerate(word_timings):
        word_text = str(w.get("word", ""))
        must_break = i in hard_break_before
        if current:
            candidate_text = " ".join(x["word"] for x in current) + " " + word_text
            width = _measure_caption_text_width(candidate_text, KARAOKE_FONT_SIZE)
            if width is not None:
                too_long = width > safe_pixel_width
            else:
                too_long = (current_len + len(word_text) + 1) > max_line_chars
            if must_break or len(current) >= words_per_line or too_long:
    ...
    family = _VIDEO_FONT_FAMILY or "DejaVu Sans"
    # ASS colours are &HAABBGGRR (alpha, blue, green, red — reversed vs. RGB).
    HIGHLIGHT = "&H0038D2FE"   # vivid amber/yellow accent, fully opaque
    BASE = "&H00FFFFFF"        # white, fully opaque

    header = (
        "[Script Info]\n"
        "ScriptType: v4.00+\n"
        f"PlayResX: {KARAOKE_PLAY_RES_X}\n"
        "PlayResY: 1920\n"
        "WrapStyle: 2\n"
        ...
        f"Style: Caption,{family},{KARAOKE_FONT_SIZE},{BASE},&H000000FF,&H00101010,&H88000000,"
        f"1,0,0,0,100,100,0.5,0,1,4,2,2,{KARAOKE_MARGIN_L},{KARAOKE_MARGIN_R},360,1\n\n"
        "[Events]\n"
    ...
    for line_idx, line_words in enumerate(lines):
        ...
        # The last word of a line gets a little extra "hang time" (below) so
        # it doesn't vanish the instant it's spoken — but that hang time must
        # never bleed into the NEXT line's first word, or both lines briefly
        # render stacked on screen at the same time.
        ...
        for idx, active in enumerate(line_words):
            ...
            for j, w in enumerate(line_words):
                ...
                if j == idx:
                    parts.append(
                        f"{{\\c{HIGHLIGHT}\\b1\\fscx112\\fscy112}}{token}"
                        f"{{\\c{BASE}\\b1\\fscx100\\fscy100}}"
                    )
                else:
                    parts.append(f"{{\\c{BASE}\\b1}}{token}")
            text = " ".join(parts)
            events.append(
    ...
    try:
        with open(ass_path, "w", encoding="utf-8") as f:
            f.write(header)
            f.write("\n".join(events))
        return os.path.exists(ass_path) and os.path.getsize(ass_path) > 0
    except Exception as e:
        log(f"⚠️ _build_karaoke_ass failed: {e}")
        return False
...
def _wrap_video_caption(text, max_chars=42):
    """Short two-line captions so viewers can actually read them on a phone."""
...
def _scene_durations(slides, total_duration):
    """Allocate time by narration length so visuals stay synced to speech."""
    n = len(slides)
    if n == 0:
        return []
    weights = [max(1, len(str(s.get("narration", "")).split())) for s in slides]
    total_words = sum(weights)
    minimum = 2.6
    durations = [max(minimum, total_duration * w / total_words) for w in weights]

    # Keep the total exactly aligned with the voice track while preserving a
    # readable minimum scene length.
    delta = sum(durations) - total_duration
    while abs(delta) > 0.02:
        if delta > 0:
            candidates = [i for i, d in enumerate(durations) if d > minimum + 0.15]
            if not candidates:
                break
            i = max(candidates, key=lambda j: durations[j])
            step = min(delta, durations[i] - minimum)
            durations[i] -= step
            delta -= step
        else:
...
def build_video_reel(slides, out_path):
    """
    Build a higher-quality 1080x1920 / 30fps Reel with:
      - speech-synced scene durations
      - varied Ken-Burns movement
      - varied cinematic transitions
      - real word-by-word karaoke-style captions, highlighted exactly in
        sync with the spoken audio (via edge-tts's native word timings +
        libass), with an automatic fallback to a clean static caption if
        word-level timing isn't available for any reason
      - richer color grade + subtle vignette + sharpening for a more
        polished, "premium" finish
      - high-quality H.264 encoding (slower preset, lower CRF = visibly
        sharper/cleaner output than before)
      - optional local/approved background music
    No additional Gemini/image API calls are made here.
    """
    if not FFMPEG_AVAILABLE:
    ...
    try:
        ...
        # Each slide's narration already gets its own trailing "." added
        # below, so scenes are joined with a plain space (NOT ". "), which
        # would otherwise double up periods between scenes ("...floor.. Res-
        # earchers...") — harmless for TTS but needlessly sloppy input text.
        scene_texts = [str(s.get("narration", "")).strip().rstrip(".!?") for s in slides]
        full_narration = " ".join(t + "." for t in scene_texts)
        # Word count per scene, using the exact text that gets spoken, so the
        # karaoke caption builder can tell where one narration sentence ends
        # and the next begins in the flat, per-word timing list TTS returns.
        scene_word_counts = [len(t.split()) for t in scene_texts]
        audio_path = os.path.join(session_dir, "voice.mp3")
        has_audio, word_timings = _generate_voiceover_with_timings(full_narration, audio_path)

        if has_audio:
            total_duration = max(8.0, min(_get_media_duration(audio_path), 55.0))
        else:
            total_duration = max(8.0, min(len(slides) * 4.2, 55.0))

        # Build the karaoke subtitle track from real TTS word timings. This is
        # independent of per-scene image durations below — the words are
        # highlighted at the exact moment they're actually spoken, regardless
        # of how the visuals are split across scenes.
        ass_path = os.path.join(session_dir, "captions.ass")
        use_karaoke_captions = bool(_VIDEO_FONT) and bool(word_timings) and _build_karaoke_ass(
            word_timings, ass_path, scene_word_counts=scene_word_counts
        )
        if word_timings and not use_karaoke_captions:
            log("⚠️ Karaoke captions unavailable this run (missing font or ASS build failure) — using static captions instead.")

        durations = _scene_durations(slides, total_duration)
        fps = 30
        transition = 0.35
        transitions = ["fade", "smoothleft", "smoothright", "wipeleft", "wiperight", "circleopen", "diagtl"]
        random.shuffle(transitions)

        # Build every scene in one FFmpeg graph so the final render is encoded
        # once at high quality rather than repeatedly transcoding clips.
        filter_parts = []
        video_labels = []
        for i, (img_path, slide, duration) in enumerate(zip(img_paths, slides, durations)):
            frames = max(1, int(round(duration * fps)))
            mode = i % 4
            if mode == 0:
                zoom = "min(max(zoom+0.0010,1.0),1.22)"
            elif mode == 1:
                zoom = "max(min(zoom-0.0010,1.22),1.0)"
            elif mode == 2:
                zoom = "min(max(zoom+0.0007,1.0),1.16)"
            else:
                zoom = "max(min(zoom-0.0007,1.16),1.0)"

            vf = [
                f"[{i}:v]scale=1180:2100:force_original_aspect_ratio=increase:flags=lanczos",
                "crop=1080:1920",
                f"zoompan=z='{zoom}':d={frames}:s=1080x1920:fps={fps}",
                "setsar=1",
                # Slightly stronger sharpening + richer, punchier color grade and a
                # gentle vignette for a more cinematic, "premium" finished look.
            ...
            # Static per-scene caption is only used as a fallback when the
            # real word-synced karaoke captions aren't available this run.
            if _VIDEO_FONT and not use_karaoke_captions:
                caption = _escape_ffmpeg_text(_wrap_video_caption(slide["narration"]))
                vf.append(
                    f"drawtext=fontfile={_VIDEO_FONT}:text='{caption}':"
                    "fontsize=50:fontcolor=white:"
                    "box=1:boxcolor=black@0.52:boxborderw=20:"
                    "shadowcolor=black@0.75:shadowx=2:shadowy=2:"
                    "x=(w-text_w)/2:y=h-470:line_spacing=8"
                )
        ...
        for i in range(1, len(video_labels)):
            ...
            out = f"vx{i}"
            offset = max(0.01, elapsed - transition)
            filter_parts.append(
                f"[{current}][{video_labels[i]}]xfade="
                f"transition={trans}:duration={transition}:offset={offset:.3f}[{out}]"
            )
            current = out
            elapsed += durations[i] - transition

        # Burn in the real word-synced karaoke captions via libass, on top of
        # the fully composited/graded video, right before final formatting.
        if use_karaoke_captions:
            escaped_ass = _escape_ffmpeg_filter_path(ass_path)
            ass_opts = f"ass=filename='{escaped_ass}'"
            if _VIDEO_FONT_DIR:
                ass_opts += f":fontsdir='{_escape_ffmpeg_filter_path(_VIDEO_FONT_DIR)}'"
            filter_parts.append(f"[{current}]{ass_opts}[vcap]")
            current = "vcap"

        # Optional small corner watermark/handle — only drawn if configured
        # (VIDEO_WATERMARK_TEXT) and a font is available; otherwise this is a
        # complete no-op and nothing about the output changes.
        if VIDEO_WATERMARK_TEXT and _VIDEO_FONT:
            safe_watermark = _escape_ffmpeg_text(VIDEO_WATERMARK_TEXT[:40])
            filter_parts.append(
                f"[{current}]drawtext=fontfile={_VIDEO_FONT}:text='{safe_watermark}':"
                "fontsize=30:fontcolor=white@0.55:"
                "x=w-text_w-28:y=48:shadowcolor=black@0.6:shadowx=1:shadowy=1[vwm]"
            )
            current = "vwm"

        # Cinematic fade-in from black / fade-out to black on the fully
        # composited frame (image + captions + watermark together) — a
        # standard finishing touch on professional short-form edits. Without
        # this the very first and last frames previously appeared/vanished
        # instantly, which reads as an unfinished cut rather than an edit.
        fade_in_dur = 0.35
        fade_out_dur = 0.45
        fade_out_start = max(elapsed - fade_out_dur, fade_in_dur)
        filter_parts.append(
            f"[{current}]fade=t=in:st=0:d={fade_in_dur}:color=black,"
            f"fade=t=out:st={fade_out_start:.3f}:d={fade_out_dur}:color=black[vfade]"
        )
        current = "vfade"
        ...
        for img_path in img_paths:
            prefix += ["-loop", "1", "-i", img_path]
        if has_audio:
            prefix += ["-i", audio_path]
        if music_enabled:
            prefix += ["-stream_loop", "-1", "-i", VIDEO_MUSIC_FILE]

        audio_filter = ""
        if has_audio:
            voice_idx = len(img_paths)
            # Same fade timing as the video, so sound and picture begin and
            # end together instead of audio cutting off under a visual fade.
            audio_fade_in_dur = 0.25
            audio_fade_out_dur = 0.5
            audio_fade_out_start = max(elapsed - audio_fade_out_dur, audio_fade_in_dur)
            if music_enabled:
                music_idx = voice_idx + 1
                # Voice stays dominant; the optional music bed is deliberately subtle.
                audio_filter = (
                    f";[{voice_idx}:a]highpass=f=70,lowpass=f=12000,"
                    "acompressor=threshold=-18dB:ratio=2.5:attack=8:release=120,"
                    "loudnorm=I=-16:TP=-1.5:LRA=11[voice];"
                    f"[{music_idx}:a]volume={max(0.0, min(VIDEO_MUSIC_VOLUME, 0.12))},"
                    "highpass=f=80,lowpass=f=9000[music];"
                    "[voice][music]amix=inputs=2:duration=first:dropout_transition=2[amix];"
                    f"[amix]afade=t=in:st=0:d={audio_fade_in_dur},"
                    f"afade=t=out:st={audio_fade_out_start:.3f}:d={audio_fade_out_dur}[aout]"
                )
            else:
                audio_filter = (
                    f";[{voice_idx}:a]highpass=f=70,lowpass=f=12000,"
                    "acompressor=threshold=-18dB:ratio=2.5:attack=8:release=120,"
                    "loudnorm=I=-16:TP=-1.5:LRA=11[avoice];"
                    f"[avoice]afade=t=in:st=0:d={audio_fade_in_dur},"
                    f"afade=t=out:st={audio_fade_out_start:.3f}:d={audio_fade_out_dur}[aout]"
                )
        ...
        if has_audio:
            final_cmd += ["-map", "[aout]", "-c:a", "aac", "-b:a", "192k", "-ar", "48000"]
        else:
            final_cmd += ["-an"]
        final_cmd += [
            "-t", f"{elapsed:.3f}",
            # Slower preset + lower CRF = visibly sharper, cleaner Reel than
            # before, at the cost of a longer (but still bounded) render time.
        ...
        r = subprocess.run(final_cmd, capture_output=True, timeout=480)
        if r.returncode != 0 or not os.path.exists(out_path) or os.path.getsize(out_path) < 10000:
            log(f"⚠️ ffmpeg final Reel build failed: {r.stderr.decode(errors='ignore')[-1200:]}")
            return False
...
def _cover_text_lines(text, max_words=8):
    ...
    wrapped by REAL rendered pixel width (not just word count) at the
    cover's 72px title size, so a line built from a few long words can't run
    off either edge of the 1080px-wide frame the way a fixed-word-count
    split risked doing."""
    ...
    title_fontsize = 72
    safe_width = (KARAOKE_PLAY_RES_X - 80) * 0.90  # frame minus side margins, with a safety margin

    def fits(candidate_words):
        joined = " ".join(candidate_words)
        w = _measure_caption_text_width(joined, title_fontsize)
        if w is not None:
            return w <= safe_width
        return len(joined) <= 21  # conservative estimate if Pillow isn't installed

    line1_words = []
    for w in words:
        if not line1_words or fits(line1_words + [w]):
    ...
    line2_words = []
    for w in remaining:
        if not line2_words or fits(line2_words + [w]):
            line2_words.append(w)
        else:
            break
    line2 = " ".join(line2_words).rstrip(".,!?;:")
    if len(line2_words) < len(remaining):
        line2 += "…"
...
def build_reel_cover(slides, cover_path):
    ...
    Visual style: a warm gold accent colour (the SAME accent used for the
    word-highlight in the in-video karaoke captions, for a consistent look
    between thumbnail and video) on a smooth bottom-up gradient — instead of
    the old flat gray box — so the photo stays vivid up top and the headline
    stays perfectly readable at the bottom, closer to a real edited
    thumbnail than plain bordered text dropped on a darkened photo.
    """
    if not FFMPEG_AVAILABLE or not slides:
        return False
    try:
        # Defensive: don't rely on the caller having created VIDEO_TEMP_DIR
        # first (generate_and_post_video does today, but this function
        # shouldn't silently break the moment that call order ever changes).
        ...
        # Same warm gold used for the karaoke word-highlight in the video
        # itself (ffmpeg 0xRRGGBB form of the ASS &H0038D2FE accent), so the
        # thumbnail and the video read as one consistent, branded piece.
        ACCENT = "0xFED238"

        vf = [
            "scale=1080:1920:force_original_aspect_ratio=increase",
            "crop=1080:1920",
            # Lighter global darkening than before (-0.10 -> -0.02) plus a
            # bit more punch (saturation/contrast) - the photo itself should
            # read as vivid and eye-catching; the gradient bands below do
            # the actual work of keeping the headline readable, instead of
            # flattening the whole frame uniformly.
            "eq=brightness=-0.02:saturation=1.22:contrast=1.14:gamma=1.02",
            "unsharp=5:5:0.4:5:5:0.0",
            "vignette=PI/5",
        ]
        if _VIDEO_FONT:
            # Smooth bottom-up gradient (transparent near the middle, dark by
            # the bottom edge) built from stacked bands, replacing the old
            # single flat box - keeps the top of the photo vivid while
            # guaranteeing strong contrast under every line of text.
            gradient_top, gradient_bottom, band_count = 620, 1920, 14
            span = gradient_bottom - gradient_top
            for i in range(band_count):
                y0 = gradient_top + round(span * i / band_count)
                y1 = gradient_top + round(span * (i + 1) / band_count)
                alpha = 0.06 + (0.72 - 0.06) * (i / (band_count - 1))
                vf.append(f"drawbox=x=0:y={y0}:w=1080:h={y1 - y0}:color=black@{alpha:.3f}:t=fill")

            # Kicker rendered as a solid gold "badge" (dark text on the
            # accent colour) instead of plain floating white text.
            vf.append(f"drawbox=x=280:y=693:w=520:h=62:color={ACCENT}@0.95:t=fill")
            vf.append(
                f"drawtext=fontfile={_VIDEO_FONT}:text='YOU NEED TO SEE THIS':fontsize=30:"
                "fontcolor=black@0.88:x=(w-text_w)/2:y=712"
            )
            vf.extend([
                f"drawtext=fontfile={_VIDEO_FONT}:text='{title_line1}':fontsize=72:fontcolor=white:x=(w-text_w)/2:y=825:borderw=3:bordercolor=black",
                f"drawtext=fontfile={_VIDEO_FONT}:text='{title_line2}':fontsize=72:fontcolor=white:x=(w-text_w)/2:y=925:borderw=3:bordercolor=black",
                # Small gold accent underline beneath the headline - a cheap,
                # safe way to add colour to the title block without the
                # legibility risk of multi-colour headline text.
                f"drawbox=x=(iw-140)/2:y=1015:w=140:h=6:color={ACCENT}@0.9:t=fill",
                f"drawtext=fontfile={_VIDEO_FONT}:text='{subtitle}':fontsize=38:fontcolor=0xFFE9B0@0.95:x=(w-text_w)/2:y=1080:borderw=1:bordercolor=black@0.6",
                # Slim gold brand stripe along the very bottom edge.
                f"drawbox=x=0:y=1906:w=1080:h=14:color={ACCENT}:t=fill",
            ])
        else:
            log("⚠️ build_reel_cover: no font available, using plain graded image (no title text) for the cover.")
...
def _wait_for_reel_ready(video_id, graph_version, max_wait_seconds=60, poll_interval=6):
    """
    Polls the video's processing status before we try to set a custom
    thumbnail. Facebook can silently ignore/reject a thumbnail uploaded
    while the Reel is still transcoding, which previously made the custom
    cover a coin-flip. Best-effort only: if the status field isn't available
    or times out, we still proceed to try the thumbnail upload once, exactly
    like before, rather than blocking the pipeline.
    """
    waited = 0
    while waited < max_wait_seconds:
        try:
            status_resp = requests.get(
                ...
                params={"fields": "status", "access_token": FB_ACCESS_TOKEN},
                timeout=REQUEST_TIMEOUT,
            ).json()
            video_status = (status_resp.get("status", {}) or {}).get("video_status")
            if video_status == "ready":
                return True
            if video_status == "error":
                log(f"⚠️ Reel {video_id} reported processing error before thumbnail upload.")
                return False
        except Exception:
            pass
    ...
    try:
        ...
        if finish_resp.get("success"):
            ...
            if cover_path and os.path.exists(cover_path):
                # Give Facebook a chance to finish processing the video first —
                # uploading a thumbnail too early is a common reason a custom
                # cover silently doesn't stick and the auto-picked frame shows
                # instead. This is bounded so it can't hang the scheduler.
                _wait_for_reel_ready(video_id, GRAPH_VERSION, max_wait_seconds=60, poll_interval=6)
                thumb_ok = False
                for attempt in range(2):
                    try:
                        ...
                        if thumb_resp.status_code not in (200, 201):
                            log(f"⚠️ Reel cover upload not accepted (attempt {attempt + 1}/2); Reel remains published: {thumb_resp.text[:300]}")
                        else:
                            thumb_json = thumb_resp.json() if thumb_resp.content else {}
                            if isinstance(thumb_json, dict) and thumb_json.get("error"):
                                log(f"⚠️ Reel cover upload rejected (attempt {attempt + 1}/2); Reel remains published: {thumb_json}")
                            else:
                                log(f"✅ Reel custom cover set for {video_id}")
                                thumb_ok = True
                    except Exception as e:
                        log(f"⚠️ Reel cover step failed (attempt {attempt + 1}/2); Reel remains published: {e}")
                    if thumb_ok:
                        break
                    time.sleep(8)
            return {"id": video_id}
