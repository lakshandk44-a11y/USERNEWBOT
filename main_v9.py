...
CATEGORY_ENABLED = {
    ...
    "quote": True,
    "fact": True,
    "poll": True,    # NEW: daily reaction poll
}
CATEGORY_LABELS = {
    "news":    "📰 News",
    "scenic":  "🏞️ Scenic",
        ...
        for update in data.get("result", []):
            ...
            if text in ("/stats", "/status"):
                ...
                send_telegram_message(
                    ...
                    "/token - check Facebook token validity & expiry\n"
                    "/pause - pause all posting and comment replies\n"
...
def call_gemini(prompt, retries=3, label="gemini"):
    ...
    NEW: if ALL Gemini attempts fail, automatically retries once via the
    Pollinations.ai free text API (no API key needed) before giving up.
    This means the bot keeps generating content even during Gemini outages
    or quota exhaustion, without any code changes elsewhere.
    """
...
1. HOOK (1 line, max 10 words): Create a curiosity gap or bold statement that
   STOPS the scroll. First 3 words are critical — shown before "See more".
   Use one of these techniques:
   - Curiosity gap: "Nobody is talking about what happened next..."
   - Shock/surprise: "This just changed everything we thought we knew."
   - Bold opinion: "This is either brilliant — or a disaster."
   - Question hook: "Why is NOBODY outraged about this?"
2. BLANK LINE
3. STORY (2-4 sentences): Explain what happened. Be specific, clear, factual.
4. BLANK LINE
5. EMOTION LINE (1-2 sentences): Why should people CARE? Tap into: hope, anger,
   surprise, inspiration, or concern. Make it personal and relatable.

For image_prompt:
- Cinematic photojournalism scene that DIRECTLY depicts the people, place, or
  event in this story — highly specific, never generic
- Dramatic lighting, ultra-detailed, 4k
- NO text, NO headlines, NO logos, NO writing in the image

Return ONLY JSON:
{{"caption":"[hook]\\n\\n[story]\\n\\n[emotion line]","image_prompt":"..."}}
""", label="ai_generate")
        ...
        try:
            ...
            if "error" not in result:
                ...
                if "id" in result:
                    ...
                    # REACH BOOST: post a self-comment question immediately after
                    # publishing to trigger early engagement signal on Facebook.
